...
let fixed = 0, scanned = 0, c8803=0, noReorder=0, stillFail=0;
for (const f of FILES) {
  ...
  for (let bi = blocks.length - 1; bi >= 0; bi--) {
    ...
    if (r0.ok) continue; if (!/error CS8803/.test(r0.log)) continue; c8803++;
    const reordered = reorder(code);
    if (!reordered) { noReorder++; continue; }
    const r1 = await compile(reordered);
    if (!r1.ok) { stillFail++; console.log("  重排后仍失败:", (r1.log.match(/error CS\d+/)||[]).slice(0,2).join(",")); continue; }
    lines.splice(s + 1, e - s - 1, ...escape(reordered).split("\n"));
    fixed++; changed = true;
  }
  if (changed) { writeFileSync(p, lines.join("\n"), "utf8"); console.log(`${f}: 已写入`); }
}
console.log(`扫描 ${scanned} | CS8803 ${c8803} | 无法重排 ${noReorder} | 重排后仍失败 ${stillFail} | 已修复 ${fixed}`);
