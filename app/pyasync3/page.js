...
// =============================================================
// Python asyncio 教程 V3（pyasync3）交互式页面
// -------------------------------------------------------------
// 专题教程：demo 驱动学 asyncio
//   核心理念：每一章都从一个可运行的完整 demo 开始，
//   通过详细注释 + 知识点拆解，让你「看代码就懂原理」。
//   1. 数据源：pyasync3Chapters / pyasync3ChapterGroups（来自 pyasync3-tutorial-data）
//   2. 运行接口：/api/run-py（由 CodeBlock 内部调用）
//   3. 底部 demo 直接复用 CodeBlock 组件，与教程内代码块完全一致
//
// 风格定位：简单简单再简单、demo 多、注释详细、讲完整讲清楚
// =============================================================

import { useState, useEffect, useRef, useCallback } from "react";
import { pyasync3Chapters, pyasync3ChapterGroups } from "../courses-data/pyasync3-tutorial-data";
...
export default function PyAsync3Tutorial() {
  ...
  const initialChapter = pyasync3Chapters[0];
  ...
  useEffect(() => {
    ...
    if (!hash) return;
    const chapter = pyasync3Chapters.find((c) => c.id === hash);
    if (chapter) {
  ...
  const groupedChapters = pyasync3ChapterGroups.map((group) => ({
    group,
    items: pyasync3Chapters.filter((c) => c.group === group),
  }));
    ...
    <div className="app-shell">
      <div className="main-layout">
        ...
        <Sidebar
          ...
          currentPath="/pyasync3"
          meta={`共 ${pyasync3Chapters.length} 章 · demo 驱动学异步`}
        />
        ...
        <main className="content" ref={contentRef}>
          ...
          <footer className="content-footer">
            <p>
              Python asyncio 教程 V3 · demo 驱动学异步 · 代码由系统 python3 子进程执行
            </p>
          </footer>
        </main>
      </div>
...
function ChapterNav({ activeId, onSelect }) {
  const idx = pyasync3Chapters.findIndex((c) => c.id === activeId);
  const prev = idx > 0 ? pyasync3Chapters[idx - 1] : null;
  const next = idx < pyasync3Chapters.length - 1 ? pyasync3Chapters[idx + 1] : null;
