...
// C++ 代码执行 API 路由
// -------------------------------------------------------------
// 作用：接收前端发送的 C++ 源代码，调用系统 clang++（或 g++）
//       编译成临时可执行文件后运行，捕获 stdout / stderr 返回前端。
//
// 与 Python / Ruby 路由的区别：
//   C++ 是编译型语言，执行流程比解释型语言多一步编译：
//     1. 将源代码写入临时 .cpp 文件
//     2. 用 clang++（优先）或 g++（回退）编译成临时可执行文件 Playground
//     3. 运行该可执行文件
//     4. 清理临时文件
//
//   二进制名固定为 Playground，避免与系统命令冲突，也便于在
//   临时目录里统一识别和清理。与 C 路由共用一套临时文件流程，
//   仅编译器和源文件后缀不同。
//
// 安全说明：
...
let _resolvedCxx = null;

/**
 * 探测可用的 C++ 编译器：优先 clang++，不可用则回退 g++。
 * 用 spawnSync --version 同步探测，命中即缓存。
 * @returns {string} 编译器可执行文件名
 */
function pickCompiler() {
  if (_resolvedCxx !== null) return _resolvedCxx;
  for (const bin of [PREFERRED_CXX, FALLBACK_CXX]) {
    try {
  ...
  _resolvedCxx = PREFERRED_CXX;
  return PREFERRED_CXX;
}
  ...
  return new Promise((resolve) => {
    ...
    child.on("error", () => {
      ...
      resolve(
        NextResponse.json({
          status: "error",
          message: `未找到 ${PREFERRED_CXX} / ${FALLBACK_CXX}，请先安装 C++ 编译器`,
          version: "",
        })
      );
    });
