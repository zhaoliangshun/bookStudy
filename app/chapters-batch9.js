...
\`npm run <script>\` 并不是简单地"运行命令"，背后有一套完整的执行流程：

1. **Shell 选择**：npm 在 Unix 系统通过 \`/bin/sh -c\` 执行脚本，在 Windows 通过 \`cmd.exe /d /s /c\` 执行，这就是跨平台差异的根源
2. **PATH 注入**：npm 启动子进程前，将 \`./node_modules/.bin\` 临时前置到 \`PATH\` 环境变量，所以脚本里可以直接写 \`eslint\`、\`webpack\` 而无需 \`./node_modules/.bin/\` 前缀
3. **环境变量扁平化**：npm 将 package.json 的字段按路径展开为 \`npm_package_*\` 变量（如 \`npm_package_scripts_build\`），并将当前事件存入 \`npm_lifecycle_event\`
4. **钩子查找**：执行 \`npm run build\` 时，npm 先在 scripts 中查找 \`prebuild\`，再执行 \`build\`，最后执行 \`postbuild\`，任一步非零退出码则中断

\`\`\`
npm run build 执行流程：
┌──────────────────────────────────────────────────────┐
│ 1. 读取 package.json → 解析 scripts 字段             │
│ 2. PATH 注入: node_modules/.bin 前置到 PATH          │
│ 3. 环境变量注入: npm_package_* / npm_lifecycle_event │
│ 4. sh -c "prebuild"  (若存在)                        │
│ 5. sh -c "build"      (主脚本)                       │
│ 6. sh -c "postbuild"  (若存在)                       │
└──────────────────────────────────────────────────────┘
\`\`\`
...
\`\`\`json
// ✗ 错误：Unix 语法在 Windows cmd 中无法识别
"start": "NODE_ENV=production node index.js"

// ✓ 正确：使用 cross-env 统一设置
"start": "cross-env NODE_ENV=production node index.js"
\`\`\`

**陷阱 3：prepublish 钩子意外触发**

\`\`\`json
// ✗ 错误：prepublish 在 npm install 时也会触发，导致安装依赖时意外构建
"prepublish": "npm run build"

// ✓ 正确：使用 prepublishOnly，仅在 npm publish 前触发
"prepublishOnly": "npm run build"
\`\`\`

**陷阱 4：& 并行符在 Windows 行为异常**

\`\`\`json
// ✗ 错误：& 在 Windows cmd 中语义不同，且无法正确传递退出码
"build": "eslint . & webpack"

// ✓ 正确：使用 npm-run-all 并行执行
"build": "npm-run-all --parallel lint build"
\`\`\`

### 「性能提示」npm 脚本性能最佳实践

**1. CI 环境使用 \`npm ci\` 加速安装**

\`npm ci\` 严格按 package-lock.json 安装，跳过版本解析和 lock 文件更新，比 \`npm install\` 快 2-5 倍，且保证确定性。CI 中应始终使用 \`npm ci\`。

\`\`\`yaml
# GitHub Actions 示例
- run: npm ci        # 而非 npm install
- run: npm run build
\`\`\`

**2. 并行执行独立任务**

测试套件、lint、构建等独立任务可用 \`npm-run-all --parallel\` 同时执行，充分利用多核 CPU，总耗时降为最慢任务的时间。

**3. 利用 npm 缓存加速安装**

\`npm install\` 默认从 \`~/.npm/_cacache\` 读取已缓存的包内容。CI 中可配置缓存目录持久化，第二次构建大幅提速。
...
npm 解析 \`"express": "^4.18.2"\` 这样的依赖范围时，内部执行一套精密的算法：

1. **范围解析**：将 \`^4.18.2\` 拆解为下界 \`>=4.18.2\` 和上界 \`<5.0.0\`（MAJOR+1）
2. **0.x 特殊处理**：\`^0.2.3\` 的上界是 \`<0.3.0\`（不是 \`<1.0.0\`），\`^0.0.3\` 的上界是 \`<0.0.4\`——0.x 和 0.0.x 视为不稳定，不兼容更新只到下一个 patch/minor
3. **依赖树解析**：npm v3+ 采用**扁平化**策略，将依赖提升到顶层 node_modules，冲突的版本才嵌套安装
4. **lock 文件锁定**：package-lock.json 记录整棵依赖树的精确版本和 integrity 哈希，\`npm ci\` 直接按此安装，跳过解析
...
^4.18.2 的范围解析：
  ...
  │ 下界: >=4.18.2                  │
  │ MAJOR≥1 → 上界: <5.0.0         │
  │ MAJOR=0,MINOR≥1 → 上界: <0.x.0 │
  │ MAJOR=0,MINOR=0 → 上界: <0.0.x+1│
  └─────────────────────────────────┘
  ↓
  结果: >=4.18.2 <5.0.0
\`\`\`
...
\`\`\`json
// ✗ 错误：package.json 写 "lodash": "^4.17.20"
// 开发者本地装到 4.17.21（修复版），但 CI 全新安装时
// 解析到 4.17.22（含破坏性变更），测试全部失败
"lodash": "^4.17.20"

// ✓ 正确：提交 package-lock.json，CI 用 npm ci 严格按锁文件安装
// 紧急情况可精确锁定版本
"lodash": "4.17.21"
\`\`\`

**陷阱 2：0.x 版本的 \`^\` 行为不符合直觉**

\`\`\`json
// ✗ 错误：误以为 ^0.2.0 允许更新到 0.3.0
"lib": "^0.2.0"   // 实际只允许 >=0.2.0 <0.3.0

// ✓ 正确：0.x 阶段视为不稳定，明确使用精确版本或 ~
"lib": "~0.2.0"   // >=0.2.0 <0.3.0（同上但语义更清晰）
"lib": "0.2.5"    // 完全锁定
\`\`\`
...
\`\`\`json
// ✗ 错误：插件要求 react@^16，但项目装了 react@18
// npm 7+ 自动安装 peer，报 ERESOLVE 错误
"plugins/react-plugin": "^1.0.0"  // peerDeps: react@^16

// ✓ 正确：升级插件到兼容 react@18 的版本，或用 --legacy-peer-deps 临时绕过
"plugins/react-plugin": "^2.0.0"  // peerDeps: react@^18
\`\`\`

**陷阱 4：lock 文件与 package.json 不一致**

\`\`\`bash
# ✗ 错误：手动改 package.json 版本但忘记重新 install
# 导致 lock 文件与 package.json 脱节，npm ci 报错

# ✓ 正确：改完 package.json 后立即 npm install 更新 lock 文件
npm install lodash@4.17.21 --save
git add package.json package-lock.json
git commit -m "chore: pin lodash"
\`\`\`

### 「性能提示」依赖管理性能最佳实践

**1. CI 使用 \`npm ci\` 加速并保证确定性**

\`npm ci\` 比 \`npm install\` 快 2-5 倍：跳过版本解析、跳过 lock 更新、先删 node_modules 再装。它还校验 lock 与 package.json 一致性，不一致直接报错，避免"在我机器上能跑"。
...
- uses: actions/cache@v3
  with:
    path: ~/.npm        # 缓存 npm 内容寻址缓存
    key: ${{ runner.os }}-npm-${{ hashFiles('**/package-lock.json') }}
- run: npm ci --prefer-offline   # 优先用缓存
\`\`\`

**3. 定期清理冗余依赖**

使用 \`npm prune\` 清除不在 package.json 中的包，\`npm dedupe\` 去重扁平化依赖树，减少 node_modules 体积和安装时间。
...
### 「底层原理」ESLint 的 AST 遍历与规则插件

ESLint 的核心是一个**基于 AST（抽象语法树）的静态分析引擎**：

1. **解析阶段**：解析器（默认 Espree）将源码转换为 ESTree 规范的 AST，每个语法节点（FunctionDeclaration、VariableDeclarator 等）都有 type 和位置信息
2. **规则注册**：每条规则通过 \`create(context)\` 返回一个**选择器→回调**的映射对象，选择器匹配 AST 节点类型
3. **遍历阶段**：ESLint 深度优先遍历 AST，遇到匹配的节点类型就调用对应规则的回调，回调通过 \`context.report()\` 上报问题
4. **配置级联**：ESLint 从当前目录向上查找 .eslintrc，逐层合并（extends 优先级从低到高，rules 覆盖 extends）
...
└──────────────────────────────┘
  ↓
遍历 AST → 匹配选择器 → 调用回调 → 收集问题 → 输出报告
\`\`\`

### 「常见陷阱」代码规范的工程化陷阱

**陷阱 1：ESLint 规则过严阻塞开发**

\`\`\`javascript
// ✗ 错误：所有规则设为 error，console.log 和未使用变量直接阻断提交
rules: {
  'no-console': 'error',
  'no-unused-vars': 'error',
}

// ✓ 正确：风格类用 warn，质量类才用 error；开发环境可适当放宽
rules: {
  'no-console': process.env.NODE_ENV === 'production' ? 'error' : 'warn',
  'no-unused-vars': 'warn',
  'no-undef': 'error',      // 真正的质量问题才 error
}
\`\`\`

**陷阱 2：ESLint 与 Prettier 规则冲突**

\`\`\`javascript
// ✗ 错误：ESLint 和 Prettier 都管引号/分号，互相覆盖，lint --fix 死循环
rules: { 'semi': ['error', 'always'], 'quotes': ['error', 'single'] }
// 同时 Prettier 配置 semi: false, singleQuote: true

// ✓ 正确：eslint-config-prettier 关闭 ESLint 的格式规则，格式全交给 Prettier
extends: ['eslint:recommended', 'prettier'],  // prettier 必须最后
rules: { /* 只留代码质量规则 */ }
\`\`\`
...
\`\`\`javascript
// ✗ 错误：以为自己的 rules 会覆盖 extends，但 plugin:react/recommended 在后
extends: ['eslint:recommended', 'plugin:react/recommended'],
rules: { 'no-unused-vars': 'off' }  // 被 react 的规则覆盖？

// ✓ 正确：extends 从低到高，最后是自己的 rules（优先级最高）
// 明确顺序，必要时用 overrides 针对文件覆盖
\`\`\`

**陷阱 4：.eslintrc 多层覆盖意外**

\`\`\`
// 项目结构
src/
  .eslintrc.json    ← 子目录配置，会与根配置合并
  utils/
    .eslintrc.json  ← 再合并一次，可能意外关闭根规则
\`\`\`
根目录设 \`root: true\` 停止向上查找，子目录配置按需添加，避免意外覆盖。

### 「性能提示」代码规范性能最佳实践

**1. 使用 lint-staged 只检查变更文件**

全量 lint 大型项目可能耗时几十秒。lint-staged 只对 git 暂存区文件运行 ESLint，提交前检查从 30s 降到 2s。
...
\`eslint --cache\` 会在目录下生成 \`.eslintcache\`，只检查自上次以来变更的文件，CI 和本地都大幅提速。

\`\`\`json
"scripts": { "lint": "eslint src/ --cache --cache-location .eslintcache" }
\`\`\`

**3. CI 中分离 lint 与测试并行执行**

将 lint、type-check、unit-test 拆为独立 CI Job 并行运行，而非串行 \`npm run lint && npm test\`，总耗时降为最慢的 Job。
...
### 「底层原理」Git Hooks 的 .hooks 目录与 husky 原理

Git Hooks 的执行机制和 husky 的接管方式：

1. **原生 Hooks 目录**：Git 在 \`.git/hooks/\` 下存放钩子脚本（如 \`pre-commit.sample\`），去掉 \`.sample\` 后缀即激活。但 \`.git/\` 不被 git 跟踪，**钩子无法随仓库分享给团队**
2. **core.hooksPath 配置**：Git 允许通过 \`git config core.hooksPath\` 指定钩子目录，husky 正是利用此机制将钩子目录指向项目内的 \`.husky/\`
3. **husky install 原理**：\`npx husky install\` 执行 \`git config core.hooksPath .husky\`，并在 \`.husky/_/husky.sh\` 放置通用包装脚本
4. **prepare 脚本自动安装**：package.json 中 \`"prepare": "husky install"\` 在 \`npm install\` 后自动执行，确保团队成员克隆后钩子自动生效
...
│ git commit 触发                                │
│   ↓                                            │
│ Git 读取 core.hooksPath (= .husky)            │
│   ↓                                            │
│ 执行 .husky/pre-commit 脚本                    │
│   ↓                                            │
│ .husky/pre-commit:                             │
│   . "$(dirname "$0")/_/husky.sh"  # 加载包装器 │
│   npx lint-staged                 # 实际命令   │
│   ↓                                            │
│ 退出码 0 → 允许提交 / 非 0 → 阻止提交          │
└───────────────────────────────────────────────┘
\`\`\`

### 「常见陷阱」Git Hooks 的工程化陷阱

**陷阱 1：Git Hook 未提交到仓库**

\`\`\`bash
# ✗ 错误：直接编辑 .git/hooks/pre-commit，其他成员无此钩子
echo "npm run lint" > .git/hooks/pre-commit
chmod +x .git/hooks/pre-commit
# .git/ 不被 git 跟踪，团队其他人的钩子不会生效

# ✓ 正确：使用 husky，钩子放在 .husky/ 目录并提交到仓库
npx husky add .husky/pre-commit "npx lint-staged"
git add .husky/pre-commit
git commit -m "chore: add pre-commit hook"
\`\`\`

**陷阱 2：pre-commit 阻塞紧急修复**

\`\`\`bash
# ✗ 错误：生产事故时 pre-commit 跑全量测试，5 分钟无法提交
git commit -m "fix: critical production bug"
# 等待 lint + test 完成，延误修复

# ✓ 正确：紧急情况用 --no-verify 跳过钩子（事后补测试）
git commit --no-verify -m "fix: critical production bug"
# 注意：CI 仍会运行完整检查，跳过的只是本地钩子
\`\`\`

**陷阱 3：husky v8 与 v9 API 不兼容**

\`\`\`bash
# ✗ 错误：husky v9 中 husky add 已废弃
npx husky add .husky/pre-commit "npm test"  # 报错
...
**陷阱 4：commit-msg 钩子获取参数方式错误**

\`\`\`bash
# ✗ 错误：误以为 commit message 通过 stdin 传入
msg=$(cat)  # 读不到内容

# ✓ 正确：Git 将 message 文件路径作为 $1 传入
npx --no -- commitlint --edit "$1"
\`\`\`

### 「性能提示」Git Hooks 性能最佳实践

**1. lint-staged 只检查暂存文件**

pre-commit 钩子若全量 lint/test，大项目耗时分钟级。lint-staged 只对 \`git diff --cached\` 的文件运行检查，配合 \`eslint --fix\` 自动修复，提交检查控制在秒级。

**2. pre-push 才跑完整测试**

将耗时操作（完整测试套件、集成测试）放到 pre-push 而非 pre-commit。提交频繁但推送较少，这样既保证质量又不影响开发节奏。

**3. 钩子脚本使用 npx --no-install**

\`\`\`bash
# .husky/pre-commit
npx --no-install lint-staged   # 只用本地已安装的版本，避免触发安装
\`\`\`
避免钩子触发意外的包下载，保证执行速度和确定性。
...
### 「底层原理」Docker 的 UnionFS 分层与 cgroups 资源限制

Docker 容器并非"轻量级虚拟机"，而是基于 Linux 内核三大隔离机制：

1. **UnionFS（联合文件系统）**：镜像由多个**只读层**叠加而成，每条 Dockerfile 指令（FROM/RUN/COPY）生成一层。容器启动时在最上层加一个**可写层**（Copy-on-Write），修改文件时才从下层复制到可写层
2. **namespaces（命名空间隔离）**：PID namespace 隔离进程、NET namespace 隔离网络、MNT namespace 隔离文件系统挂载、UTS namespace 隔离主机名
3. **cgroups（资源限制）**：控制 CPU 时间片、内存上限、磁盘 I/O、网络带宽，防止单容器耗尽宿主机资源

\`\`\`
Docker 镜像分层结构（Copy-on-Write）：
┌──────────────────────────┐
│ 可写层 (容器层)           │ ← docker run 产生，删除容器即消失
├──────────────────────────┤
│ CMD ["node","index.js"]  │ ← 第 5 层 (只读)
├──────────────────────────┤
│ COPY . .                 │ ← 第 4 层 (只读)
├──────────────────────────┤
│ RUN npm ci               │ ← 第 3 层 (只读, 缓存)
├──────────────────────────┤
│ COPY package*.json ./    │ ← 第 2 层 (只读, 缓存)
├──────────────────────────┤
│ FROM node:20-alpine      │ ← 第 1 层 (基础镜像)
└──────────────────────────┘
\`\`\`

### 「常见陷阱」Docker 容器化的工程化陷阱

**陷阱 1：Docker 镜像过大**

\`\`\`dockerfile
# ✗ 错误：用完整镜像 + 装了 devDependencies + 保留源码
FROM node:20          # ~900MB
COPY . .
RUN npm install       # 含 devDeps
# 最终镜像 1.3GB+

# ✓ 正确：Alpine 基础 + 多阶段构建 + 仅生产依赖
FROM node:20-alpine AS builder   # ~50MB
RUN npm ci
FROM node:20-alpine
COPY --from=builder /app/dist ./dist
RUN npm ci --only=production
...
\`\`\`dockerfile
# ✗ 错误：先 COPY 源码再 npm install，改一行代码就重装依赖
COPY . .
RUN npm ci           # 源码变了，这层缓存失效，每次都重装

# ✓ 正确：先 COPY package.json 再 install，最后 COPY 源码
COPY package*.json ./
RUN npm ci           # package.json 不变就命中缓存
COPY . .             # 源码变更只影响这层
\`\`\`
...
\`\`\`dockerfile
# ✗ 错误：默认 root 运行，攻击者拿到 root 危害宿主机
CMD ["node", "index.js"]
...
**陷阱 4：未处理 SIGTERM 导致优雅关闭失败**

\`\`\`javascript
// ✗ 错误：不监听 SIGTERM，docker stop 后等 10s 才 SIGKILL，请求中断
server.listen(3000);

// ✓ 正确：监听 SIGTERM，关闭 HTTP 连接后退出
process.on('SIGTERM', () => {
  server.close(() => process.exit(0));
});
\`\`\`

### 「性能提示」Docker 容器化性能最佳实践

**1. 多阶段构建减小镜像体积**

构建阶段使用完整镜像（含编译工具、devDeps），运行阶段仅 COPY 产物到 Alpine 镜像。镜像从 1.3GB 降到 ~100MB，拉取和启动速度提升 10 倍，攻击面也大幅缩小。

**2. 优化层缓存命中率**

将不常变的内容（基础镜像、package.json、依赖安装）放在 Dockerfile 前部，常变的源码放在后部。配合 \`.dockerignore\` 排除 node_modules/dist/.git，减少构建上下文传输量。
...
\`\`\`
BuildKit 支持多阶段并行构建、缓存挂载（\`--mount=type=cache\`）、更智能的层复用，构建速度提升 30-50%。
...
### 「底层原理」PM2 的 cluster 模式与 IPC 通信

PM2 的核心能力建立在 Node.js 内置的 cluster 模块和进程间通信（IPC）之上：

1. **cluster 模式原理**：主进程（Master）通过 \`cluster.fork()\` 创建多个工作进程（Worker），所有 Worker 共享同一个 TCP 端口（SO_REUSEPORT）。Master 使用 Round-Robin 算法将连接分发到各 Worker
2. **IPC 通道**：Master 与 Worker 之间通过内置的 IPC 管道通信，PM2 借此传递 \`process.env\`、接收状态上报、发送 reload/restart 信号
3. **零停机重载机制**：\`pm2 reload\` 逐个重启 Worker——先 fork 新 Worker → 等新 Worker 触发 \`listening\` 事件（端口就绪）→ 向旧 Worker 发 SIGINT → 旧 Worker 优雅关闭 → 始终有 Worker 在线
4. **God Daemon**：PM2 守护进程（God）自身独立运行，监管所有应用进程，崩溃后自动 fork 新实例

\`\`\`
PM2 cluster 模式架构：
                    ┌─────────────┐
   客户端请求 ──────→│  PM2 Master │ (God Daemon)
                    │  Round-Robin │
                    └──────┬──────┘
              ┌────────┬───┴────┬────────┐
              ↓        ↓        ↓        ↓
...
reload 流程（零停机）：
  fork 新W2 → W2 listening → SIGINT 旧W2 → 旧W2 close → 重复下一个
\`\`\`

### 「常见陷阱」PM2 进程管理的工程化陷阱

**陷阱 1：reload 与 restart 误用**

\`\`\`bash
# ✗ 错误：生产环境用 restart，所有进程同时重启，服务中断
pm2 restart api-server   # 瞬间全部掉线，请求 502

# ✓ 正确：cluster 模式用 reload，逐个重启，零停机
pm2 reload api-server    # 始终有进程在线
# 注意：fork 模式只有单进程，reload 等同于 restart，仍会中断
\`\`\`

**陷阱 2：cluster 模式下 Session/状态丢失**

\`\`\`javascript
// ✗ 错误：Session 存在进程内存，请求被分发到不同 Worker，登录状态丢失
const sessions = {};  // 内存 Map，各 Worker 独立

// ✓ 正确：Session 存到 Redis，所有 Worker 共享
...
**陷阱 3：未配置 max_memory_restart 导致内存泄漏**

\`\`\`javascript
// ✗ 错误：Node.js 内存泄漏逐渐累积，进程 OOM 崩溃后才重启，请求中断
// ecosystem.config.js 无 max_memory_restart

// ✓ 正确：设置内存阈值，超限自动重启
max_memory_restart: '500M'   // 超过 500MB 自动重启该 Worker
\`\`\`

**陷阱 4：SIGTERM 未处理导致优雅关闭失败**

\`\`\`javascript
// ✗ 错误：reload 时旧进程被 SIGINT，但未监听信号，连接被强制中断
server.listen(3000);

// ✓ 正确：监听 SIGINT/SIGTERM，停止接收新连接，等现有请求完成
process.on('SIGINT', () => {
  server.close(() => process.exit(0));
});
// 配合 kill_timeout: 5000 给 5s 优雅关闭时间
\`\`\`

### 「性能提示」PM2 进程管理性能最佳实践

**1. cluster 模式充分利用多核 CPU**

\`instances: 'max'\` 自动按 CPU 核心数启动 Worker，吞吐量随核心数线性增长。4 核服务器性能提升约 3.5 倍（少量调度开销）。

\`\`\`javascript
// ecosystem.config.js
exec_mode: 'cluster',
instances: 'max',        // 或指定数字如 4
\`\`\`

**2. graceful reload 实现零停机部署**

\`pm2 reload\` 逐个重启 Worker，配合应用监听 \`process.on('SIGINT')\` 和 \`server.close()\`，实现部署期间零请求丢失。设置 \`kill_timeout\` 给应用足够时间完成存量请求。

**3. 日志轮转防止磁盘打满**

\`\`\`bash
pm2 install pm2-logrotate
pm2 set pm2-logrotate:max_size 10M       # 单文件超 10MB 轮转
pm2 set pm2-logrotate:retain 30          # 保留 30 个日志文件
pm2 set pm2-logrotate:compress true      # 旧日志 gzip 压缩
\`\`\`
避免日志文件无限增长导致磁盘打满、服务崩溃。
