...
// React 源码构建教程页面
// -------------------------------------------------------------
// 主题：从零开始一步步构建 React 核心源代码
// 含代码编辑器与运行按钮，demo 在服务端 Node.js 沙箱中执行。
// 结构：侧边栏章节导航 + 主内容区 Markdown 渲染 + 代码块。
// =============================================================

import { useState, useEffect, useRef, useCallback } from "react";
import { reactsrcChapters, reactsrcChapterGroups } from "../courses-data/reactsrc-tutorial-data";
import { MarkdownRenderer } from "../MarkdownRenderer";
import Sidebar from "../components/Sidebar";
import CodeBlock from "../CodeBlock";

import { useReadingScrollPosition } from "../hooks/useReadingScrollPosition";
export default function ReactsrcTutorial() {
  // ---------- 状态管理 ----------
  // 默认使用第一个章节作为初始状态。
  ...
  const initialChapter = reactsrcChapters[0];
  ...
  // 当前章节对象
  const activeChapter =
    reactsrcChapters.find((c) => c.id === activeId) || reactsrcChapters[0];
  ...
  useEffect(() => {
    ...
    if (!hash) return;
    const chapter = reactsrcChapters.find((c) => c.id === hash);
    if (chapter) {
  ...
  const selectChapter = useCallback((chapterId) => {
    const chapter = reactsrcChapters.find((c) => c.id === chapterId);
    if (!chapter) return;
  ...
  // 按分组组织章节
  const groupedChapters = reactsrcChapterGroups.map((group) => ({
    group,
    items: reactsrcChapters.filter((c) => c.group === group),
  }));
    ...
    <div className="app-shell">
      <div className="main-layout">
        ...
        <Sidebar
          ...
          currentPath="/reactsrc"
          meta={`共 ${reactsrcChapters.length} 章 · 源码构建`}
        />
        ...
        <main className="content" ref={contentRef}>
          ...
          <footer className="content-footer">
            <p>
              React 源码构建 · 从零构建核心源代码 · 代码在服务端沙箱中执行
            </p>
          </footer>
        </main>
      </div>
...
function ChapterNav({ activeId, onSelect }) {
  const idx = reactsrcChapters.findIndex((c) => c.id === activeId);
  const prev = idx > 0 ? reactsrcChapters[idx - 1] : null;
  const next = idx < reactsrcChapters.length - 1 ? reactsrcChapters[idx + 1] : null;
