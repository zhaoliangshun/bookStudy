...
//   2. 运行接口：/api/run-csharp（由 CodeBlock 内部调用，调用 dotnet 编译运行）
...
export default function CsharpTutorial() {
  ...
  const initialChapter = csharpChapters[0];
  ...
  useEffect(() => {
    ...
    if (!hash) return;
    const chapter = csharpChapters.find((c) => c.id === hash);
    if (chapter) {
      ...
      <div className="main-layout">
        ...
        <main className="content" ref={contentRef}>
          ...
          <section className="lesson-section">
            ...
            <CodeBlock code={activeChapter.code} lang="cs" maxHeight={400} />
          </section>
