...
理解这个三层结构：
- \`repeat_decorator(3)\` 先被调用，返回 \`decorator\`（一个真正的装饰器函数）
- \`decorator(say)\` 被调用，返回 \`wrapper\`
- \`say\` 最终指向 \`wrapper\`

所以 \`@repeat_decorator(3)\` 等价于 \`say = repeat_decorator(3)(say)\`。最外层的 \`repeat_decorator\` 是「装饰器工厂」，它生产出真正的装饰器。
...
utc = datetime.utcnow()    # ⚠️ 已弃用：datetime.utcnow() 自 Python 3.12 起弃用，推荐改用 datetime.now(timezone.utc)
print(now)
