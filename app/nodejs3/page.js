...
export default function Nodejs3Tutorial() {
  ...
  const getInitialChapterId = () => {
    if (typeof window !== "undefined") {
      ...
      if (hash && nodejs3Chapters.find((c) => c.id === hash)) {
        return hash;
      }
    }
    return nodejs3Chapters[0].id;
  };

  const initialId = getInitialChapterId();
  const initialChapter = nodejs3Chapters.find((c) => c.id === initialId) || nodejs3Chapters[0];
      ...
      <div className="main-layout">
        ...
        <main className="content" ref={contentRef}>
          ...
          <footer className="content-footer">
            <p>
              Node.js 源码手写与设计模式实战 · 从零实现核心模块与经典模式 · 代码在服务端沙箱中执行
            </p>
          </footer>
        </main>
      </div>
...
function ChapterNav({ activeId, onSelect }) {
  ...
  const next = idx < nodejs3Chapters.length - 1 ? nodejs3Chapters[idx + 1] : null;
