...
// =============================================================
// Python 多线程入门（pythread2）交互式页面
// -------------------------------------------------------------
// 专题教程：专注讲解 threading 多线程的工作原理与日常开发应用
//   1. 数据源：pythread2Chapters / pythread2ChapterGroups（来自 pythread2-tutorial-data）
...
import { pythread2Chapters, pythread2ChapterGroups } from "../courses-data/pythread2-tutorial-data";
import { MarkdownRenderer } from "../MarkdownRenderer";
import Sidebar from "../components/Sidebar";
import CodeBlock from "../CodeBlock";

import { useReadingScrollPosition } from "../hooks/useReadingScrollPosition";
export default function PyThread2Tutorial() {
  // 默认使用第一个章节作为初始状态。
  ...
  const initialChapter = pythread2Chapters[0];
  ...
  // 当前章节对象
  const activeChapter =
    pythread2Chapters.find((c) => c.id === activeId) || pythread2Chapters[0];
  ...
  useEffect(() => {
    ...
    if (!hash) return;
    const chapter = pythread2Chapters.find((c) => c.id === hash);
    if (chapter) {
  ...
  const selectChapter = useCallback((chapterId) => {
    const chapter = pythread2Chapters.find((c) => c.id === chapterId);
    if (!chapter) return;
  ...
  // 按分组组织章节
  const groupedChapters = pythread2ChapterGroups.map((group) => ({
    group,
    items: pythread2Chapters.filter((c) => c.group === group),
  }));
    ...
    <div className="app-shell">
      <div className="main-layout">
        ...
        <Sidebar
          ...
          currentPath="/pythread2"
          meta={`共 ${pythread2Chapters.length} 章 · 多线程原理 + 实战 demo`}
        />
        ...
        <main className="content" ref={contentRef}>
          ...
          <footer className="content-footer">
            <p>
              Python 多线程入门 · 代码由系统 python3 子进程执行 · 简单易懂讲原理，demo 驱动学实战
            </p>
          </footer>
        </main>
      </div>
...
function ChapterNav({ activeId, onSelect }) {
  const idx = pythread2Chapters.findIndex((c) => c.id === activeId);
  const prev = idx > 0 ? pythread2Chapters[idx - 1] : null;
  const next = idx < pythread2Chapters.length - 1 ? pythread2Chapters[idx + 1] : null;
