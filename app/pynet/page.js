...
// =============================================================
// Python 网络编程教程（pynet）交互式页面
// -------------------------------------------------------------
// 系统讲解 Python 网络编程，涵盖 socket、TCP、UDP、HTTP、HTTPS、WebSocket。
//   1. 数据源：pynetChapters / pynetChapterGroups（来自 pynet-tutorial-data）
...
export default function PyNetTutorial() {
  // ---------- 状态管理 ----------
  // 默认使用第一个章节作为初始状态。
  ...
  const initialChapter = pynetChapters[0];
  ...
  // 当前章节对象
  const activeChapter =
    pynetChapters.find((c) => c.id === activeId) || pynetChapters[0];
  ...
  useEffect(() => {
    ...
    if (!hash) return;
    const chapter = pynetChapters.find((c) => c.id === hash);
    if (chapter) {
  ...
  const selectChapter = useCallback((chapterId) => {
    const chapter = pynetChapters.find((c) => c.id === chapterId);
    if (!chapter) return;
  ...
  // 按分组组织章节
  const groupedChapters = pynetChapterGroups.map((group) => ({
    group,
    items: pynetChapters.filter((c) => c.group === group),
  }));
    ...
    <div className="app-shell">
      <div className="main-layout">
        ...
        <Sidebar
          ...
          currentPath="/pynet"
          meta={`共 ${pynetChapters.length} 章 · 网络编程`}
        />
        ...
        <main className="content" ref={contentRef}>
          ...
          <footer className="content-footer">
            <p>
              Python 网络编程教程 · 代码由系统 python3 子进程执行 · 涵盖 socket/TCP/UDP/HTTP/HTTPS/WebSocket
            </p>
          </footer>
        </main>
      </div>
...
function ChapterNav({ activeId, onSelect }) {
  const idx = pynetChapters.findIndex((c) => c.id === activeId);
  const prev = idx > 0 ? pynetChapters[idx - 1] : null;
  const next = idx < pynetChapters.length - 1 ? pynetChapters[idx + 1] : null;
