...
- **Promise 读取流程**：React 在当前 Fiber 的 \`pendingBailouts\` 集合中记录已经见过的 Promise。当 \`use(promise)\` 调用时，如果 Promise 已 resolve，直接返回值。如果未 resolve，检查是否已经抛出过：没抛过就 throw promise，Suspense 捕获后开始等待；抛过就继续挂起。Promise resolve 后，React 会重新渲染这个分支，此时就能拿到值。
- **Context 读取流程**：传统 \`useContext\` 会在 Hook 链表中存一个「订阅者」引用，用于变化时强制更新。\`use(context)\` 不存引用，它每次渲染都直接从 Fiber 树向上查找最近 Provider —— 和 Class 组件中 \`contextType\` 的查找方式类似。由于 Context 变化会导致所有消费它的祖先重渲染，查找结果永远是最新的，不需要缓存。
- **throw 机制与 Suspense 交互**：React 渲染过程本质是一个递归遍历 Fiber 树的循环。如果某一层组件 throw 了 Promise，React 会停止当前深度遍历，往上找最近的 Suspense 边界。找到后，把这个 Suspense 标记为「挂起」，渲染 fallback，剩下的子树暂时不处理。Promise resolve 后，React 重新从 Suspense 边界开始渲染，这次就能拿到值继续往下走。

## 常见陷阱

- **\`use(promise)\` 会导致组件挂起，必须有 Suspense 覆盖**：如果上层没有 Suspense，Promise 未 resolve 时会渲染失败，整个应用都会挂掉。确保任何可能抛出 Promise 的 \`use()\` 调用上层都有 Suspense。
- **每次重渲染都会重新调用 \`use(promise)\`，容易重复请求**：如果写成 \`use(fetchPost(id))\`，每次重渲染都会发起新请求。用 \`useMemo\` 缓存 Promise：\`const postPromise = useMemo(() => fetchPost(id), [id]); use(postPromise);\`
- **错误处理还是需要 ErrorBoundary**：如果 Promise reject，\`use()\` 会抛出错误，必须由 ErrorBoundary 捕获。不要指望 Suspense 处理错误，它只处理加载状态。
- **不要把 \`use()\` 用在事件处理中**：渲染过程外调用 \`use()\` 没有意义，React 无法捕获 throw 且找不到当前 Fiber。只能在渲染过程（组件函数或 Hook 函数）中使用。

## 性能提示

- **批量请求可以利用 Promise.all 并发**：多个 \`use()\` 会依次抛出，但 Promise.all 可以让多个请求并发等待：
  \`\`\`jsx
  const [a, b] = use(Promise.all([fetchA(), fetchB()]));
  \`\`\`
  这样只触发一次 Suspense fallback，而不是两次。
- **提前发起请求，配合 React 渲染流水线**：在路由变化时就可以发起请求，而不是等组件渲染才开始。服务端渲染场景下，提前预取数据可以让 HTML 直接包含渲染结果。
- **Context 频繁变化时，\`use()\` 比 \`useContext\` 略慢**：因为每次渲染都要重新从 Fiber 树向上查找，而 \`useContext\` 是从 Hook 缓存直接读。对于非常频繁变化的 Context，还是推荐用 \`useContext\`。
- **循环中使用时，尽量保持调用稳定**：虽然允许循环，但每次循环次数变化都意味着不同的读取路径，React 需要重新处理挂起状态。合理拆分 Suspense 边界可以避免整个页面 fallback。
`,
...
`
  ...
  // =============================================================
  // 第二章：ref 作为 prop（不再需要 forwardRef）
  // =============================================================
  {
    id: "react19-ref-prop",
...
- **JSX 编译阶段**：\`ref\` 不再被特殊提取，和其他 prop 一起放进 props 对象
- **协调阶段**：对于原生 DOM 元素（如 \`<input>\`），React 仍然会特殊处理 ref 赋值——在 commit 阶段把 DOM 节点赋值给 ref.current 或调用 ref callback
- **对于自定义组件**：如果组件函数接收 \`ref\` prop，就直接传进去，React 不做额外处理；如果不接收，什么也不做
- **cleanup 处理**：commit 阶段，当节点需要更新或卸载时，React 检查旧的 ref 是不是函数，并且这个函数在上一次调用时返回了另一个函数——如果是，就调用返回的那个函数做清理

所以核心变化只有一个：**把 ref 从 "特殊提取出来当第二个参数" 改成 "留在 props 里"**，没有更多魔法了。

## 常见陷阱

- **不要同时用 forwardRef 和 ref prop**：如果你已经用了 forwardRef，就不要再在 props 里声明 \`ref\`，会导致重复接收。迁移时要彻底把 forwardRef 去掉。
- **解构 props 时不要漏掉 ref**：以前不用解构 ref（因为它在第二个参数），现在别忘了：\`function Component({ children, ref }) { ... }\`
- **高阶组件要透传 ref prop**：以前 HOC 透传 ref 需要用 \`React.forwardRef\` 转发。现在直接透传 props 就行：\`function HOC(props) { return <InnerComponent {...props} />; }\`，ref 自然就透过去了。
- **cleanup 函数只有 ref callback 才有**：\`useRef\` 创建的 ref 对象，不会调用 cleanup，只有当你传函数作为 ref 时才生效。如果你用 ref 对象，清理逻辑还是要放在 \`useEffect\` 里。

## 性能提示

- **内联 ref callback 每次渲染都会重新执行**：和以前一样，如果你写成 \`<div ref={() => {}} />\`，每次重渲染都会先调用旧函数的 cleanup，再调用新函数。如果不需要 cleanup，用 \`useCallback\` 缓存可以避免重复执行。
- **利用 cleanup 减少 useEffect 代码**：需要测量 DOM、监听滚动、绑定第三方库时，直接在 ref callback 里做并返回清理函数。减少了一个 useEffect 的依赖处理，代码更紧凑，性能更好（少一次副作用执行）。
- **第三方 UI 库迁移：逐步替换 forwardRef**：不需要一次性全改，可以保持 forwardRef 同时导出，兼容新旧用法。TypeScript 中可以用交叉类型同时支持两种方式，不会 break 不兼容。
- **ref 就是普通 prop，可以正常做解构默认值**：\`function Button({ ref = null }) { ... }\` 完全没问题。以前不能这么做，因为 ref 不在 props 里。
`,
...
- **渲染阶段收集**：当 React 在渲染过程中遇到 \`<title>\`、\`<meta>\`、\`<link>\` 这些特殊标签时，它不会真的在当前位置创建 DOM 节点，而是先把它们收集到一个全局队列里。
- **commit 阶段提升**：整个树渲染完成后，React 会把收集到的所有 metadata 元素依次处理，合并相同 key 的标签（比如后面的 \`<title>\` 会覆盖前面的），然后一次性更新到 \`document.head\`。
- **去重和覆盖策略**：React 用标签的属性来判断是不是同一个 metadata：\`<title>\`：整个文档只保留最后一个。\`<meta>\`：如果有相同的 \`name\` 或 \`property\`，后面的覆盖前面的。\`<link>\`：如果有相同的 \`rel\` 和 \`href\`，去重保留一个。
- **清理**：当渲染 metadata 的组件卸载时，React 会自动从 head 中移除对应的标签，如果还有其他组件提供了同名标签，会恢复成那个版本。
...
- **多个组件都写 title，哪个生效？**：最后渲染的那个 title 生效，后面的覆盖前面的。如果嵌套路由，子页面的 title 会覆盖父页面的，符合预期。
- **viewport meta 标签写错位置会导致问题**：通常 viewport 只需要在 HTML 模板里写一次就够了。如果组件里又写了一个，会覆盖模板里的，可能导致缩放错误。
- **react-helmet 还能用吗？**：能用，React 19 不破坏它，它仍然会工作。新项目可以直接用原生支持，减少一个依赖。
- **自定义 meta 属性能用吗？**：能用，任何属性都支持，React 只会提升标签，不会校验属性。自定义 data-* 属性完全没问题。
- **\`<link rel="stylesheet">\` 也会提升到 head，对吗？**：对，link 标签不管 rel 是什么都会提升。如果你想在 body 里放 link，这做不到，所有 link 都会去 head。

## 性能提示

- **把公共 metadata 放在根组件一次写好**：比如 viewport、charset 这些全局不变的，放在 HTML 模板里就行，不用每个组件都写。只把页面级可变的 metadata（title、description）放在页面组件里。
- **避免同一个 key 在多个组件中同时渲染**：虽然覆盖机制能工作，但每次更新都需要修改 head，带来不必要的 DOM 操作。同一个 name 的 meta 只在一个地方定义最好。
- **预加载关键资源**：利用 \`<link rel="preload">\` 在 JS 加载前就开始加载字体、图片等关键资源。React 原生提升比手动注入 head 更可靠，不会遗漏。
- **卸载会自动清理，不需要手动清理**：当组件卸载，React 自动移除它贡献的 metadata，恢复之前的状态。路由切换时，前一个页面的 title 会自动被后一个页面替换，正常工作。
`,
...
- **依赖分析**：分析每个组件、每个 Hook 的数据流，找出哪些值在重渲染时是稳定的（引用不会变），哪些会变
- **memo 插入**：在需要 memo 的地方自动插入等价的 memo 包装，确保只有依赖变化时才重新计算
- **保留语义**：保证自动生成的 memo 和你手动写的行为完全一致，不会改变程序逻辑
...
- **React Compiler 不能解决所有性能问题**：它只能解决「不必要重渲染」问题。如果问题出在算法复杂度太高、大量 DOM 操作，还是需要手动优化。
- **依赖可变外部变量还是有问题**：如果你的代码依赖组件外部的全局变量，编译器无法追踪变化。还是要把可变数据放到组件内部或 props 里。
- **不要为了让编译器优化改变代码结构**：编译器能处理绝大多数常见写法，不需要刻意写"编译器友好"的代码。保持代码可读性更重要。
- **开发环境编译会变慢一点**：因为要做静态分析，编译时间会比不用时稍长。这是正常的，生产环境还是会做压缩优化，运行时没有额外开销。
- **不是所有项目都需要**：小型项目本来就没多少重渲染，收益不大。大型项目、复杂交互的项目收益最明显。

## 性能提示

- **尽早在项目中启用**：React Compiler 已经稳定，React 19 推荐默认启用。越早用，越少手动写 memo，代码越干净。
- **保留必要的手动优化**：非常昂贵的计算（比如大数组排序、复杂计算），仍然可以手动加 useMemo。编译器不会移除你写的手动 memo，双重保险也没问题。
- **不要写代码迎合编译器**：正常写你的代码，编译器会处理。刻意拆分表达式反而可能让分析更困难。
- **配合 React 19 其他新特性**：React Compiler + use() API，让你写更少的模板代码，编译器自动优化性能。开发体验提升非常明显。
`,
...
- **不要预加载用不到的资源**：预加载会占用带宽，如果预加载了很多不用的资源，反而让关键资源变慢。只预加载你"很可能马上用到"的资源。
- **preload 不代表立即缓存**：预加载是告诉浏览器提前加载，加载完会放在内存缓存里。如果很长时间不用，浏览器可能会把它从缓存中淘汰，白预加载了。
- **as 属性不能错**：\`preload\` 必须传正确的 \`as\`，否则浏览器不知道怎么优先级排队。字体一定要加 crossOrigin，否则会被加载两次（CORS 问题）。
- **API 在 react-dom，不是 react**：所有预加载 API 都从 \`react-dom\` 导出，不是从 \`react\` 导出。因为这些都是浏览器 DOM 相关的操作。
- **预加载不能跨越域名限制**：只能预加载允许跨域访问的资源，和正常加载一样受 CORS 限制。

## 性能提示

- **路由级预加载**：用户点击路由链接时，立即预加载下一个页面需要的关键资源。在跳转动画过程中，资源已经在加载了，跳转完成直接能用，体验非常流畅。
- **预判用户行为预加载**：用户鼠标悬停在一个链接/按钮上，就开始预加载对应资源。通常用户悬停 100-200ms 后会点击，这时候预加载已经开始，等点击完成加载得差不多了。
...
- **大组件懒加载 + 预加载结合**：组件懒加载后，在用户交互触发前就开始预加载。既保持了初始包小，又能快速打开。
- **第三方域名一定要 preconnect**：如果你的页面会请求 CDN、API、第三方支付等不同域名，提前 preconnect。TLS 握手 + DNS 解析节省几百毫秒，对用户感知提升明显。
- **字体预加载提升首屏体验**：自定义字体一定要预加载，避免字体闪烁（FOIT）。只预加载首屏用到的字重和字符集，不要预加载所有字体。
`,
