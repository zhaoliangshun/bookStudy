...
export default function NoderunTutorial() {
  // ---------- 状态管理 ----------
  // 默认使用第一个章节作为初始状态。
  ...
  const initialChapter = noderunChapters[0];
  ...
  // 当前章节对象
  const activeChapter =
    noderunChapters.find((c) => c.id === activeId) || noderunChapters[0];
  ...
  useEffect(() => {
    ...
    if (!hash) return;
    const chapter = noderunChapters.find((c) => c.id === hash);
    if (chapter) {
  ...
  const selectChapter = useCallback((chapterId) => {
    const chapter = noderunChapters.find((c) => c.id === chapterId);
    if (!chapter) return;
  ...
  // 按分组组织章节
  const groupedChapters = noderunChapterGroups.map((group) => ({
    group,
    items: noderunChapters.filter((c) => c.group === group),
  }));
    ...
    <div className="app-shell">
      <div className="main-layout">
        ...
        <Sidebar
          ...
          currentPath="/noderun"
          meta={`共 ${noderunChapters.length} 章 · 运行原理`}
        />
        ...
        <main className="content" ref={contentRef}>
          ...
          <footer className="content-footer">
            <p>
              Node.js 运行原理 · 层层揭示底层运行机制 · 代码在服务端沙箱中执行
            </p>
          </footer>
        </main>
      </div>
...
function ChapterNav({ activeId, onSelect }) {
  const idx = noderunChapters.findIndex((c) => c.id === activeId);
  const prev = idx > 0 ? noderunChapters[idx - 1] : null;
  const next = idx < noderunChapters.length - 1 ? noderunChapters[idx + 1] : null;
