...
//   1. 数据源：httpChapters / httpChapterGroups（来自 http-tutorial-data）
//   2. 运行接口：/api/run（由 CodeBlock 内部调用）
...
export default function HTTPTutorial() {
  ...
  const initialChapter = httpChapters[0];
  ...
  // 当前章节对象
  const activeChapter =
    httpChapters.find((c) => c.id === activeId) || httpChapters[0];
  ...
  useEffect(() => {
    ...
    if (!hash) return;
    const chapter = httpChapters.find((c) => c.id === hash);
    if (chapter) {
  ...
  const selectChapter = useCallback((chapterId) => {
    const chapter = httpChapters.find((c) => c.id === chapterId);
    if (!chapter) return;
  ...
  // 按分组组织章节
  const groupedChapters = httpChapterGroups.map((group) => ({
    group,
    items: httpChapters.filter((c) => c.group === group),
  }));
    ...
    <div className="app-shell">
      <div className="main-layout">
        ...
        <Sidebar
          ...
          currentPath="/http"
          meta={`共 ${httpChapters.length} 章 · HTTP/HTTPS/HTTP2/HTTP3 全覆盖`}
        />
        ...
        <main className="content" ref={contentRef}>
          ...
          <footer className="content-footer">
            <p>
              HTTP 通信教程 · 代码由 Node.js 沙箱执行 · 从 HTTP/1.1 到 HTTP/3，讲原理也讲实战
            </p>
          </footer>
        </main>
      </div>
...
function ChapterNav({ activeId, onSelect }) {
  const idx = httpChapters.findIndex((c) => c.id === activeId);
  const prev = idx > 0 ? httpChapters[idx - 1] : null;
  const next = idx < httpChapters.length - 1 ? httpChapters[idx + 1] : null;
