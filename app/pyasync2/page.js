...
// =============================================================
// Python asyncio 教程 V2（pyasync2）交互式页面
// -------------------------------------------------------------
// 专题教程：全新视角讲解 Python asyncio 异步编程
//   1. 数据源：pyasync2Chapters / pyasync2ChapterGroups（来自 pyasync2-tutorial-data）
...
export default function PyAsync2Tutorial() {
  // 默认使用第一个章节作为初始状态。
  ...
  const initialChapter = pyasync2Chapters[0];
  ...
  // 当前章节对象
  const activeChapter =
    pyasync2Chapters.find((c) => c.id === activeId) || pyasync2Chapters[0];
  ...
  useEffect(() => {
    ...
    if (!hash) return;
    const chapter = pyasync2Chapters.find((c) => c.id === hash);
    if (chapter) {
  ...
  const selectChapter = useCallback((chapterId) => {
    const chapter = pyasync2Chapters.find((c) => c.id === chapterId);
    if (!chapter) return;
  ...
  // 按分组组织章节
  const groupedChapters = pyasync2ChapterGroups.map((group) => ({
    group,
    items: pyasync2Chapters.filter((c) => c.group === group),
  }));
  ...
  return (
    <div className="app-shell">
      <div className="main-layout">
        ...
        <Sidebar
          title="学习目录"
          tip="点击章节开始学习 Python asyncio 异步编程 V2"
          ...
          currentPath="/pyasync2"
          meta={`共 ${pyasync2Chapters.length} 章 · 协程 + 实战 demo`}
        />
        ...
        <main className="content" ref={contentRef}>
          ...
          <footer className="content-footer">
            <p>
              Python asyncio 教程 V2 · 代码由系统 python3 子进程执行 · 协程并发讲原理，demo 驱动学实战
            </p>
          </footer>
        </main>
      </div>
...
function ChapterNav({ activeId, onSelect }) {
  const idx = pyasync2Chapters.findIndex((c) => c.id === activeId);
  const prev = idx > 0 ? pyasync2Chapters[idx - 1] : null;
  const next = idx < pyasync2Chapters.length - 1 ? pyasync2Chapters[idx + 1] : null;
