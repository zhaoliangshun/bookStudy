...
// 重要修复：顶级语句中类型声明（class/struct/record/enum）必须放可执行代码之后，避免 CS8803。
// =============================================================
...
// 静态工具类（用得最多）
//   File        —— 文件读写/复制/删除/移动（适合单次操作）
//   Directory   —— 目录创建/删除/遍历（适合单次操作）
//   Path        —— 路径字符串拼接（不碰磁盘，纯字符串运算）

// 实例类（精细控制、多次复用）
//   FileInfo    —— 单文件的元数据 + 操作（多次操作同一文件时用，省 stat 开销）
...
// ⭐ 为什么不用 + "/" +？不同操作系统路径分隔符不同，Path.Combine 自动适配
...
// ⭐ 所有文件操作都用 Path.GetTempPath() 下的临时目录——避免沙箱/权限问题
...
{
    // 2. 一次性写入全部文本（覆盖）
    // ⚠️ WriteAllText 适合小文件——它一次性把所有内容加载到内存然后写入
    // 大文件（几百MB以上）请用 Stream（下一章讲），否则内存会爆
...
// 4. 一次性读取全部文本
// ⚠️ ReadAllText 会把整个文件读进内存！1GB 文件就占 1GB+ 内存
...
File.Copy(file, copy, overwrite: true);        // ⭐ overwrite 必须显式传，默认 false 抛异常
...
// FileInfo 缓存元数据，多次操作同一文件时省去重复 stat（系统调用）
// ⭐ 什么时候用 FileInfo？需要多次读取属性（Length、CreationTime 等）或多次操作同一文件时
// 单次操作用静态 File 类更简洁
...
// ⭐ FileInfo 懒刷新：缓存可能过期（其他进程修改了文件），调用 Refresh() 重新获取元数据
fi.Refresh();
\`\`\`
...
// 5. 递归搜索所有子目录
// ⚠️ SearchOption.AllDirectories 遇到无权限目录会抛 UnauthorizedAccessException
// 大目录或可能有权限问题时，用 EnumerateFiles 逐个处理更安全
...
// GetFiles：一次性返回数组，目录大时占内存（比如几十万个文件会分配大数组）
// EnumerateFiles：⭐ 惰性枚举，遍历到才读，适合大目录——返回 IEnumerable，不一次性加载所有
// 为什么 EnumerateFiles 省内存？它一个一个返回文件，不用构造包含所有文件的数组
...
// 局部函数（local function）不用移动——顶级语句规则只针对类型声明
void Log(string msg)
{
    // 追加一行带时间戳的日志
...
// 1. 写入：以字节为单位
// ⭐ using 块为什么重要？FileStream 持有文件句柄（操作系统资源），
// 不及时释放会导致文件被锁定，其他进程无法访问。
// using 确保即使发生异常，Dispose() 也会被调用，释放句柄。
using (var fs = new FileStream(file, FileMode.Create, FileAccess.Write))
{
    byte[] data = Encoding.UTF8.GetBytes("Hello Stream 你好");
    fs.Write(data, 0, data.Length);
    // using 块结束自动 Dispose，等价于 fs.Close() + 释放句柄
}
...
using (var fs = new FileStream(file, FileMode.Open, FileAccess.Read))
{
    ...
    // ⭐ 为什么用 n？最后一次 Read 可能读不满 buffer，必须用返回值截断
...
fs2.WriteByte(66);  // 'B'
// 当前作用域（方法/代码块）结束时自动 Dispose

// 写法 3：try-finally（编译器等价展开——using 本质就是语法糖）
FileStream? fs3 = null;
try
{
...
using (var sw = new StreamWriter(file, append: false))
{
    ...
    sw.WriteLine("第二行");
    sw.WriteLine("第三行");
    // StreamWriter 有缓冲区——离开 using 块时自动 Flush + Dispose
}

// 逐行读取（⭐ 处理大文本文件首选——一次只占一行内存）
// 为什么不用 ReadAllLines？因为那会一次性把所有行读进字符串数组
// StreamReader.ReadLine 循环处理 10GB 日志文件，内存永远只有一行大小
...
// 为什么？Write 之后 Position 在末尾，Read 从末尾开始读，什么都读不到
// 这是新手最常踩的坑！
ms.Position = 0;

byte[] buf = new byte[16];
int n = ms.Read(buf, 0, buf.Length);
Console.WriteLine($"读到 {n} 字节: {Encoding.UTF8.GetString(buf, 0, n)}");

// 也可以直接 ToArray() 拿全部（返回副本，修改不影响内部缓冲区）
byte[] all = ms.ToArray();
Console.WriteLine($"全部 {all.Length} 字节");

// GetBuffer() vs ToArray()：
// GetBuffer() 返回底层数组，可能比实际数据大（有未使用空间），性能高但不安全
// ToArray() 返回精确长度的副本，安全但有拷贝开销
\`\`\`
...
// 写入
using (var fs = new FileStream(file, FileMode.Create))
using (var bw = new BinaryWriter(fs)) // 嵌套 using：bw 先 Dispose，再 fs
{
    bw.Write(42);                 // int（4字节）
    bw.Write(3.14);               // double（8字节）
    bw.Write("张三");             // 字符串（带长度前缀，7位编码长度）
    bw.Write(true);               // bool（1字节）
}

// 读取（必须按写入顺序、类型对应——顺序错了数据全乱）
...
// ⭐ 分块复制：核心模式——这就是所有流复制的底层原理
// 缓冲区选多大？8KB~64KB 是常见选择：
//   - 太小（如1字节）：频繁系统调用，慢
//   - 太大（如1MB）：单次拷贝内存多，且没有性能收益
//   - 8192（8KB）是 .NET 默认值，经验证性能最优
using var inStream = new FileStream(src, FileMode.Open);
using var outStream = new FileStream(dst, FileMode.Create);

byte[] buffer = new byte[8192];   // 8KB 缓冲区
int bytesRead;
// ⭐ 关键：Read 返回值 > 0 才表示读到数据，返回 0 说明到末尾了
// 最后一次读取的字节数可能小于 buffer.Length，必须用 bytesRead 写入
...
// 异步按行读
// await foreach 是 C# 8+ 异步流语法，逐行返回不阻塞
...
> ⭐ ASP.NET Core / WinUI 等高并发场景必须用 async——同步 IO 在线程池里会拖慢整体吞吐。为什么？线程池线程数有限，同步等待IO时线程被阻塞，其他请求无法处理。
...
// 局部函数不用移动（CS8803 只限制类型声明）
async Task CopyAsync(string from, string to)
{
    // useAsync: true 启用 Windows IOCP（异步IO），真异步而不是线程池伪装
    // bufferSize: 80KB 是大文件复制的经验最优值
    ...
    using var outS = new FileStream(to, FileMode.Create, FileAccess.Write, FileShare.None,
                                    bufferSize: 81920, useAsync: true);
    await inS.CopyToAsync(outS);   // ⭐ CopyToAsync 内置分块循环，不用手写
}
...
> 生产代码请用 \`CsvHelper\` NuGet 包处理引号、转义、换行等细节。这里演示原理。
...
// ⭐ CS8803 顶级语句规则：
// using 指令 -> 可执行代码 -> 类型声明（class/record/struct/enum）
// 为什么？顶级语句编译后生成 Main 方法体，方法内不能先定义类型再执行语句。
// 局部函数可以放任何位置，不受此限制。

var user = new User { Id = 1, Name = "张三", Email = "z*@***********" };

// ⭐ 序列化：对象 -> JSON 字符串
// 为什么选 System.Text.Json 而不是 Newtonsoft.Json？
//   - System.Text.Json：.NET 内置，性能高（约2-5x快）、内存占用低，默认行为严格
//   - Newtonsoft.Json (Json.NET)：功能极其丰富、动态类型支持好、灵活度高，但需装NuGet
//   - 新项目默认推荐 System.Text.Json；需要动态/灵活转换时用 Newtonsoft
string json = JsonSerializer.Serialize(user);
Console.WriteLine(json);
// {"Id":1,"Name":"张三","Email":"z*@***********"}

// ⭐ 反序列化：JSON 字符串 -> 对象
// 返回 User?（可空引用类型）：因为 JSON 可能为 null 或格式不正确导致失败
User? parsed = JsonSerializer.Deserialize<User>(json);
Console.WriteLine($"{parsed?.Id} {parsed?.Name} {parsed?.Email}");

// ========== 类型声明放在可执行代码之后（CS8803 规则）==========
// ⭐ 为什么属性必须是 public？
// System.Text.Json 默认只序列化 public 实例属性，internal/private/静态属性会被忽略
public class User
{
    public int Id { get; set; }
    public string Name { get; set; } = ""; // = "" 避免 null 引用警告
...
// ⭐ 推荐做法：复用同一个 Options 实例
// 为什么？JsonSerializerOptions 内部有缓存（属性访问器、类型映射等），
// 每次 new 都会重建缓存，性能差很多。Web API 中应注册为单例。
var options = new JsonSerializerOptions
{
    WriteIndented = true,                       // 缩进美化（调试/配置文件用，生产关闭省字节）
    PropertyNamingPolicy = JsonNamingPolicy.CamelCase,  // 驼峰命名（Web API 标准风格：userName）
...
public class Product
{
    ...
    public string? Description { get; set; }   // nullable 引用类型
}
\`\`\`
...
有时候 JSON 字段名和 C# 属性名就是不一致（比如对接第三方 API 用 snake_case）：
...
// 反序列化时同样按 user_id 匹配 UserId（JsonPropertyName 双向生效）
var back = JsonSerializer.Deserialize<ApiResponse>(json);
Console.WriteLine($"反序列化成功: {back?.UserName}");

// ========== 类型声明放最后 ==========
public class ApiResponse
{
    [JsonPropertyName("user_id")]    // 序列化/反序列化都用这个名字
...
public class Account
{
    ...
    [JsonIgnore]                       // 永远不序列化/反序列化（密码等敏感字段必加）
    public string Password { get; set; } = "";

    [JsonIgnore(Condition = JsonIgnoreCondition.WhenWritingNull)]
    public string? Phone { get; set; } // 仅 null 时不输出，有值时正常输出

    [JsonIgnore(Condition = JsonIgnoreCondition.WhenWritingDefault)]
    public int Age { get; set; }       // 默认值 0 时不输出（值类型默认值不输出）
}
\`\`\`

> 安全提示：\`JsonIgnore\` 是默认推荐方式，比 \`[NonSerialized]\` 更明确，且对 System.Text.Json 直接生效。
...
var order = new Order
{
    ...
    Customer = new User { Id = 1, Name = "张三", Email = "z*@*com" },
    Items = new()
    {
...
// ========== 所有类型声明都放在可执行代码之后 ==========
// User 类在同一代码块内重新定义（因为顶级语句中每个代码块独立编译）
public class User
{
    public int Id { get; set; }
    public string Name { get; set; } = "";
...
> \`System.Text.Json\` 自动处理任意深度的嵌套——只要类型有 public 无参构造函数（或支持构造函数反序列化）。
...
// ⚠️ Dictionary<int, T> 也会把 int key 转成 JSON 字符串（如 "1"、"2"）
...
// ========== record 类型声明放最后 ==========
// record 主构造函数：参数名要和属性名匹配（System.Text.Json 支持构造函数反序列化）
// ⭐ .NET 8 的 System.Text.Json 完美支持 record，主构造函数参数自动参与序列化
public record Point(int X, int Y);

public record UserDto(int Id, string Name)
{
    public string? Email { get; init; } // init-only 属性，只能在初始化时设置
};
\`\`\`
...
// JsonDocument 是只读 DOM，用完即弃，性能高（零分配解析）
// ⭐ using var 为什么？JsonDocument 租用共享缓冲区，必须 Dispose 归还，否则内存泄漏
...
// 遍历数组（EnumerateArray 不分配集合，惰性枚举）
...
// 写入配置文件
// ⭐ 配置文件缩进好看，生产API响应关闭WriteIndented省带宽
var opts = new JsonSerializerOptions { WriteIndented = true };
...
var resp = new PagedResponse<User>
{
    ...
    Items = new()
    {
        new() { Id = 1, Name = "张三", Email = "z*@*com" },
        new() { Id = 2, Name = "李四", Email = "l*@*com" },
    }
};

// Web API 标准风格：驼峰 + 缩进（开发环境）+ 忽略null
...
- 嵌套对象/集合/字典自动序列化。
- 不想定义类用 \`JsonDocument\`（只读高性能）或 \`JsonNode\`（可变 DOM）。
...
string input = "我的电话是 138-1234-5678，邮箱是 z*@***********";

// 1. IsMatch：是否匹配（返回 bool，适合验证场景）
bool hasPhone = Regex.IsMatch(input, @"\\d{3}-\\d{4}-\\d{4}");
Console.WriteLine($"有电话? {hasPhone}");   // True

// 2. Match：返回第一个匹配（提取单个结果）
...
// 3. Matches：返回所有匹配（MatchCollection，提取多个结果）
MatchCollection ms = Regex.Matches(input, @"[a-z]+@[a-z.]+");
foreach (Match x in ms) Console.WriteLine($"邮箱: {x.Value}");

// 4. Replace：替换（批量替换敏感信息、格式化文本）
string masked = Regex.Replace(input, @"\\d{4}", "****");
Console.WriteLine(masked);   // 我的电话是 138-****-****，邮箱是 z*@***********

// 5. Split：按模式分割（比 string.Split 灵活，支持多种分隔符）
...
// ?  前面的元素出现 0 次或 1 次（可选）
Console.WriteLine(Regex.IsMatch("color", "colou?r"));    // True（美式拼写）
Console.WriteLine(Regex.IsMatch("colour", "colou?r"));   // True（英式拼写）

// ^  字符串开头；$  字符串结尾（锚点，不消费字符）
Console.WriteLine(Regex.IsMatch("hello", "^hello"));    // True
Console.WriteLine(Regex.IsMatch("hello world", "^hello$")); // False（后面还有内容）
...
// \\s 空白字符（空格、tab、换行等）；\\S 非空白
Console.WriteLine(Regex.IsMatch("a b", @"a\\sb"));     // True
\`\`\`

要点：
- C# 中正则字符串用 \`@"..."\` 逐字字符串，否则 \`\\\\d\` 要写四个反斜杠（C# 字符串转义 + 正则转义）。
...
// [abc]   匹配 a 或 b 或 c（任一字符）
Console.WriteLine(Regex.IsMatch("bat", "[bcd]at"));    // True
Console.WriteLine(Regex.IsMatch("cat", "[bcd]at"));    // True
Console.WriteLine(Regex.IsMatch("mat", "[bcd]at"));    // False

// [a-z]   范围：小写字母
Console.WriteLine(Regex.IsMatch("hello", "^[a-z]+$")); // True

// [^abc]  取反：不是 a/b/c（^ 在字符类开头表示取反）
Console.WriteLine(Regex.IsMatch("xyz", "^[^abc]+$"));  // True

// [0-9a-fA-F]  十六进制字符（多个范围组合）
...
// 为什么不写成一个正则？一个正则写密码规则可读性差，难调试
// 拆分多个小正则，每个规则一个 IsMatch，清晰易维护
bool StrongPwd(string s) =>
    Regex.IsMatch(s, @"^[A-Za-z0-9]{6,18}$")  // 长度6-18，只能是字母数字
    && Regex.IsMatch(s, @"[A-Za-z]")            // 至少一个字母
    && Regex.IsMatch(s, @"\\d");                 // 至少一个数字
Console.WriteLine(StrongPwd("abc12345"));    // True
Console.WriteLine(StrongPwd("abcdef"));      // False（无数字）
\`\`\`
...
// 用 () 分组：年-月-日——分组后可以从 Groups 中提取各部分
Match m = Regex.Match(input, @"(\\d{4})-(\\d{2})-(\\d{2})");
if (m.Success)
{
    Console.WriteLine($"完整匹配: {m.Value}");    // 2024-01-15（Groups[0]）
...
// 非捕获分组 (?:...)：分组但不存储到 Groups，性能略好
// 什么时候用？只需要分组优先级，不需要提取内容时——节省内存和捕获开销
Match m2 = Regex.Match("abc123def", @"(?:[a-z]+)(\\d+)(?:[a-z]+)");
Console.WriteLine(m2.Groups[1].Value);  // 123（只捕获了数字组）
\`\`\`
...
// 命名分组：(?<名称>模式)——用名字访问 Groups，比数字索引可读性高太多
...
// 替换时引用命名分组：\${name} 语法（C# 正则替换语法，不是 JS 插值）
// 比 $1 $2 这种数字引用清晰，不怕分组顺序变了
...
> ⭐ 命名分组在解析结构化文本（日志、协议）时远比索引可读——生产代码首选命名分组。
...
// Singleline：让 . 也匹配 \\n（默认不匹配换行符）
// 名字有点反直觉：Singleline 不是「单行模式」，而是让 . 匹配一切包括换行
string html = "<div>hello\\nworld</div>";
Match m = Regex.Match(html, @"<div>(.*?)</div>", RegexOptions.Singleline);
Console.WriteLine(m.Groups[1].Value);  // hello\\nworld（不加Singleline只匹配到hello）

// Compiled：编译为 IL 代码，多次调用更快（启动慢、占内存）
// 什么时候用？正则会被调用成百上千次（如循环内处理几万条数据）
// 只调用一两次别用，编译开销反而更大
...
// 设置超时：防止「灾难性回溯」（catastrophic backtracking）把 CPU 跑满
// 为什么会有灾难性回溯？某些嵌套量词模式（如 (a+)+）在输入不匹配时会指数级尝试所有可能
try
{
...
// ⭐ 生产代码**必须**给正则设置超时！否则恶意构造的输入可以让你的服务 CPU 100%（ReDoS 攻击）

// ========== 类型声明（partial 类）放在可执行代码之后 ==========
// ⭐ .NET 7+ 源生成器：编译时生成正则代码，启动快、运行快、零反射、无启动开销
// 比 RegexOptions.Compiled 还好：Compiled 是运行时编译，源生成器是编译时生成
// 在 partial 类/方法上标注 [GeneratedRegex] 即可
...
> 生产代码**必须**给正则设超时——某些模式匹配长字符串会指数级回溯，瞬间 100% CPU（ReDoS 攻击）。
...
string text = "电话 138******78，备用 139******21";

// C# 9+ 高性能 API（.NET 6+ 正式可用）
// 为什么快？返回 struct 枚举，不分配 MatchCollection 和 Match 对象，零 GC 压力
...
> \`ValueMatch\` 只有 \`Index\`/\`Length\`，不包含 \`Groups\`。需要分组捕获时还是得用 \`Matches\`。
...
// 常用正则模式（业务系统中可以提取为静态工具类复用）

// 邮箱：粗略校验——先过滤明显非法的，最终还是要发验证邮件确认
// ⚠️ 不要试图用正则完美匹配 RFC 5322——那个规则有几千字符，可读性为0，维护噩梦
...
Console.WriteLine(IsEmail("z**@***********"));      // True
Console.WriteLine(IsEmail("bad-email"));            // False
Console.WriteLine(IsPhoneCN("138******78"));        // True
Console.WriteLine(IsPhoneCN("12345678901"));        // False（第二位是2）
...
// 用命名分组解析日志——结构化日志解析的经典用法
// 模式用 + 拼接可读性更高，注意 \\s+ 匹配任意空白
...
// 反向引用 \\1：引用第一个捕获组匹配的内容（匹配重复单词）
Console.WriteLine(Regex.IsMatch("hello hello", @"^(\\w+) \\1$"));   // True（hello重复）
Console.WriteLine(Regex.IsMatch("hello world", @"^(\\w+) \\1$"));   // False

// 先行断言 (?=...)：匹配「后面是...」的位置（不消费字符，零宽断言）
// 提取数字后面紧跟字母的那些数字
...
// 否定先行 (?!...)：匹配「后面不是...」的位置
// 找后面不跟数字的字母
foreach (Match m in Regex.Matches("a1 b2 c", @"[a-z](?!\\d)"))
    Console.WriteLine(m.Value);   // c（c后面是字符串结尾，不是数字）
\`\`\`
