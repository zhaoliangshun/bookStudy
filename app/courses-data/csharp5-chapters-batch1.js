...
// 本批包含 6 章：
//   csharp5-preface : 前言
//   csharp5-ch01    : 第一章 .NET 与 C# 总览
//   csharp5-ch02    : 第二章 开发环境搭建
//   csharp5-ch03    : 第三章 第一个 C# 程序
//   csharp5-ch04    : 第四章 顶级语句与程序结构
//   csharp5-ch05    : 第五章 控制台输入输出
//
...
本书起草时按 92 讲（加前言、结语共 94 篇）规划目录；**当前线上成书为 128 篇**（前言 + 126 讲正文 + 结语），覆盖语言基础到生产交付：
...
站点目录以**线上 128 篇**为准（前言 + 126 讲 + 结语），不要把本前言草稿里的「94 篇 / 92 讲」当成现网篇数。交互 demo 仍按 \`net8.0\` / C# 12 可运行；C# 13/14 与 .NET 10 API 会在正文里单独标注。

### 九、和站点目录对齐

前言文件在运行时会被过滤，不进入正式目录，但它仍是仓库里的版本说明。若你从 git 历史看到「94 篇」，以站点侧栏 **128 篇** 为准：前言 1 篇 + 正文 126 讲 + 结语 1 篇。练习仍建议每讲做完再往下；跳读时先看该讲小结表。

学的时候把本前言当「版本与节奏说明」，把侧栏 128 篇当「现在该读哪一讲」。两者不一致时，信侧栏。正文从第一章的运行时总览开始，按顺序读即可。
`,
    code: `// ===========================================================
// 第〇章 前言 —— 入门示例
