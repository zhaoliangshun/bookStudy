...
- **生命周期提醒**：.NET 9（STS）的支持已于 2026 年 5 月结束；.NET 8（LTS）将于 2026 年 11 月结束支持。现有系统应尽快制定升级计划。
...
// 2) 目标数字：生产写法可用 Random.Shared.Next(1, 101) 随机生成
//    （Random.Shared 是 .NET 6+ 推荐的线程安全随机数生成器）。
//    这里用固定值 50，保证空输入的沙箱演示也能快速验证逻辑。
int target = 50;
