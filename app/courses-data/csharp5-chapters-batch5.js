...
\`base()\` 必须是构造函数体之前的第一件事（或主构造转发）。构造顺序是：**派生类字段初始化器 → 基类字段初始化器 → 基类构造体 → 派生类构造体**。注意字段初始化器先于基类构造体执行，所以基类构造里调用 virtual 时，派生类的字段初始化器**已经跑完**，还没执行的是派生类的**构造函数体**——如果派生类在构造函数体里（而非字段初始化器里）赋初值，虚方法就会看到默认值。这是继承里最阴的坑之一。
