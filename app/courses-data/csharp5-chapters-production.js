...
- 正文中的 \`bash\` / \`sql\` / \`xml\` / \`csharp\` 片段若依赖外部宿主，只作语义说明；不完整的真实框架片段会标成 \`csharp-snippet\`，避免点「运行」失败。能否在你的机器上运行，以该生态的真实项目为准。
...
你不需要记住每一行 API，但需要知道回哪一章：语法在第一～十三部分，生产主线在第十四～十八部分，专题在第十九～二十部分，生产深水区在第二十一部分，答辩在第一百一十六章。建议在毕业仓库的 \`docs/map.md\` 里只留三列：主题、本书标题章号、你们自己的 runbook 链接。章节会过时，你们的链接要跟着补丁和事故更新。
