...
`,
    code: `// ============================================================
// 输入验证三个生产习惯（纯 C# 模拟）：
//   1. 聚合全部字段错误：一次返回所有问题，不让客户端逐个试错
//   2. 错误分流：格式问题 400，领域冲突 409——code 是稳定契约
//   3. 入站 DTO 白名单：只把允许客户端写的字段拷进实体，防 over-posting
// ============================================================
...
`,
    code: `// ============================================================
// 幂等 + keyset 分页（纯 C# 模拟，Dictionary 只是教学存储；
// 生产用数据库唯一索引 + 行状态约束实现同样语义）
// ============================================================
...
`,
    code: `// ============================================================
// 浏览器安全三件事（纯 C# 模拟）：
//   1. 魔数识别：扩展名和 Content-Type 都能伪造，文件头不能
//   2. Origin 白名单：跨域放行是显式列表，不是通配符
//   3. 文件名消毒：路径穿越与保留名在上传入口就处理掉
// ============================================================
...
`,
    code: `// ============================================================
// Generic Host 的三个核心概念（最小容器模拟）：
//   1. 三种生命周期：Singleton / Scoped / Transient 的实例边界
//   2. 捕获依赖：Singleton 攥住 Scoped 是经典生产事故
//   3. Keyed 解析（.NET 8+）：同接口多实现按 key 区分
// ============================================================
...
`,
    code: `// ============================================================
// 缓存击穿防护（单飞 single-flight，纯 C# 模拟）：
//   1. 热点 key 过期瞬间，N 个并发请求只放行 1 个回源
//   2. 回源异常必须清理在途记录，否则后续请求全部卡死
// ============================================================
...
`,
    code: `// ============================================================
// 健康检查两级模型（纯 C# 模拟）：
//   1. liveness 只看进程自身——活着 ≠ 依赖都好
//   2. readiness 只看关键依赖——非关键挂了降级而不是拒流量
//   3. 三级汇总：Healthy / Degraded / Unhealthy
// ============================================================
...
`,
    code: `// ============================================================
// Aspire 编排的两个核心思想（纯 C# 模拟）：
//   1. 资源引用注入：应用只依赖资源名，连接信息由环境提供
//   2. 启动拓扑：WaitFor 表达依赖，编排器按拓扑序拉起资源
// ============================================================
...
`,
    code: `// ============================================================
// 多租户隔离的三条防线（纯 C# 模拟）：
//   1. 查询过滤：忘记 tenant 条件就会串数据
//   2. 租户来源：只信已验证 token，不信客户端 header
//   3. 缓存 key：必须带租户前缀，裸 key = 跨租户共享
// ============================================================
...
`,
    code: `// ============================================================
// 背压 + deadline（真实 Channel，可直接运行）：
//   有界容量 + Wait 模式：队列满时生产者被拖住
//   叠加 deadline：到点取消入队，而不是无限堆积
// ============================================================
await RunAsync();
...
`,
    code: `// ============================================================
// 合规日志的两个层次（纯 C# 模拟）：
//   1. 机密字段脱敏：邮箱、手机保留首尾可排障，中间打码
//   2. 严格机密直接占位：密码、证件连脱敏版本都不该出现
// ============================================================
