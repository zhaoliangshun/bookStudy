// =============================================================
// C# 从入门到精通大全（终极版）—— 第12批章节
// 第十二部分 日期时间与网络（共 6 章）
// -------------------------------------------------------------
// 本批包含 6 章：
//   csharp3-ch61 : 第六十一章 DateTime 与 TimeSpan
//   csharp3-ch62 : 第六十二章 DateTimeOffset、DateOnly、TimeOnly 与时区
//   csharp3-ch63 : 第六十三章 Uri 与 URL 处理
//   csharp3-ch64 : 第六十四章 HttpClient 基础
//   csharp3-ch65 : 第六十五章 HttpClient 进阶（JSON、Header、重试）
//   csharp3-ch66 : 第六十六章 Socket 与 TCP/UDP 编程
//
...
const chapters = [
  // ============================================================
  // 第六十一章：DateTime 与 TimeSpan
  // ============================================================
  {
    id: 'csharp3-ch61',
    group: '第十二部分 日期时间与网络',
    icon: '📅',
    title: '第六十一章 DateTime 与 TimeSpan',
    content: `## 第六十一章　DateTime 与 TimeSpan

日期时间是日常开发的基础。本章讲解 \`DateTime\`，\`TimeSpan\`，以及时区最佳实践。

### 一、DateTime 基础 ⭐⭐⭐

\`\`\`csharp\n// 1. 当前时间\nDateTime now = DateTime.Now;          // 本地时间（含时区）\nDateTime utcNow = DateTime.UtcNow;    // UTC 时间（推荐）\nDateTime today = DateTime.Today;      // 今天 0 点\nDateTime min = DateTime.MinValue;     // 0001-01-01\nDateTime max = DateTime.MaxValue;     // 9999-12-31\n\nConsole.WriteLine($\"Now      = {now:yyyy-MM-dd HH:mm:ss}\");\nConsole.WriteLine($\"UTC      = {utcNow:yyyy-MM-dd HH:mm:ss}\");\nConsole.WriteLine($\"Today    = {today:yyyy-MM-dd}\");\nConsole.WriteLine($\"Min      = {min}\");\nConsole.WriteLine($\"Max      = {max}\");\n\n// 2. 构造特定时间\nDateTime d1 = new DateTime(2026, 7, 18);                   // 日期\nDateTime d2 = new DateTime(2026, 7, 18, 14, 30, 0);        // 日期 + 时间\nDateTime d3 = new DateTime(2026, 7, 18, 14, 30, 0, DateTimeKind.Utc); // UTC\nConsole.WriteLine($\"d2 = {d2}\");\n\n// 3. 解析字符串\nDateTime parsed = DateTime.Parse(\"2026-07-18 14:30:00\");\nDateTime exact = DateTime.ParseExact(\"20260718\", \"yyyyMMdd\", null);\nConsole.WriteLine($\"parsed = {parsed}\");\nConsole.WriteLine($\"exact  = {exact}\");\n\n// 4. 安全解析\nif (DateTime.TryParse(\"2026-07-18\", out DateTime result))\n    Console.WriteLine($\"OK: {result}\");\nelse\n    Console.WriteLine(\"解析失败\");\n\nif (DateTime.TryParseExact(\"20260718\", \"yyyyMMdd\", null,\n    System.Globalization.DateTimeStyles.None, out DateTime r2))\n    Console.WriteLine($\"OK2: {r2}\");\n\`\`\`\n\n### 二、DateTime 属性 ⭐⭐⭐\n\n\`\`\`csharp\nDateTime d = new DateTime(2026, 7, 18, 14, 35, 22, 123);\n\nConsole.WriteLine($\"Year    = {d.Year}\");\nConsole.WriteLine($\"Month   = {d.Month}\");\nConsole.WriteLine($\"Day     = {d.Day}\");\nConsole.WriteLine($\"Hour    = {d.Hour}\");\nConsole.WriteLine($\"Minute  = {d.Minute}\");\nConsole.WriteLine($\"Second  = {d.Second}\");\nConsole.WriteLine($\"Ms      = {d.Millisecond}\");\nConsole.WriteLine($\"DayOfWeek = {d.DayOfWeek}\");\nConsole.WriteLine($\"DayOfYear = {d.DayOfYear}\");\nConsole.WriteLine($\"Date    = {d.Date}\");\nConsole.WriteLine($\"TimeOfDay = {d.TimeOfDay}\");\nConsole.WriteLine($\"Ticks   = {d.Ticks}\");\n\n// 季度\nint quarter = (d.Month - 1) / 3 + 1;\nConsole.WriteLine($\"Quarter = Q{quarter}\");\n\n// 是否闰年\nbool isLeap = DateTime.IsLeapYear(d.Year);\nConsole.WriteLine($\"{d.Year} 闰年? {isLeap}\");\n\n// 月份天数\nint days = DateTime.DaysInMonth(d.Year, d.Month);\nConsole.WriteLine($\"{d.Year}-{d.Month} 有 {days} 天\");\n\`\`\`\n\n### 三、DateTime 计算 ⭐⭐⭐\n\n\`\`\`csharp\nDateTime d = new DateTime(2026, 7, 18, 14, 30, 0);\n\nConsole.WriteLine($\"原   = {d}\");\nConsole.WriteLine($\"+1天 = {d.AddDays(1)}\");\nConsole.WriteLine($\"-1天 = {d.AddDays(-1)}\");\nConsole.WriteLine($\"+3h = {d.AddHours(3)}\");\nConsole.WriteLine($\"-30m= {d.AddMinutes(-30)}\");\nConsole.WriteLine($\"+1月 = {d.AddMonths(1)}\");\nConsole.WriteLine($\"-1年= {d.AddYears(-1)}\");\n\n// 月初/月末\nDateTime firstOfMonth = new DateTime(d.Year, d.Month, 1);\nDateTime endOfMonth = firstOfMonth.AddMonths(1).AddDays(-1);\nConsole.WriteLine($\"月初 = {firstOfMonth:yyyy-MM-dd}\");\nConsole.WriteLine($\"月末 = {endOfMonth:yyyy-MM-dd}\");\n\n// 季初/季末\nint q = (d.Month - 1) / 3 * 3 + 1;\nDateTime qStart = new DateTime(d.Year, q, 1);\nDateTime qEnd = qStart.AddMonths(3).AddDays(-1);\nConsole.WriteLine($\"季初 = {qStart:yyyy-MM-dd}\");\nConsole.WriteLine($\"季末 = {qEnd:yyyy-MM-dd}\");\n\`\`\`\n\n### 四、TimeSpan：时间间隔 ⭐⭐⭐\n\n\`\`\`csharp\n// 1. 创建\nTimeSpan t1 = new TimeSpan(1, 30, 0);     // 1h30m\nTimeSpan t2 = TimeSpan.FromHours(1.5);    // 1.5h\nTimeSpan t3 = TimeSpan.FromMinutes(90);\nTimeSpan t4 = TimeSpan.FromDays(1.5);\n\nConsole.WriteLine($\"t1 = {t1}\");\nConsole.WriteLine($\"t2 = {t2}\");\nConsole.WriteLine($\"t3 = {t3}\");\nConsole.WriteLine($\"t4 = {t4}\");\n\n// 2. 零和边界\nConsole.WriteLine($\"Zero = {TimeSpan.Zero}\");\nConsole.WriteLine($\"MaxValue = {TimeSpan.MaxValue}\");\nConsole.WriteLine($\"MinValue = {TimeSpan.MinValue}\");\n\n// 3. 属性\nConsole.WriteLine($\"Days={t4.Days}, Hours={t4.Hours}, Minutes={t4.Minutes}, Seconds={t4.Seconds}\");\nConsole.WriteLine($\"TotalHours={t4.TotalHours}, TotalDays={t4.TotalDays}\");\n\n// 4. 运算\nTimeSpan sum = TimeSpan.FromHours(1) + TimeSpan.FromMinutes(30);\nTimeSpan diff = TimeSpan.FromHours(2) - TimeSpan.FromMinutes(15);\nTimeSpan mul = TimeSpan.FromMinutes(10) * 3;\nTimeSpan div = TimeSpan.FromHours(2) / 4;\n\nConsole.WriteLine($\"1h+30m = {sum}\");\nConsole.WriteLine($\"2h-15m = {diff}\");\nConsole.WriteLine($\"10m*3  = {mul}\");\nConsole.WriteLine($\"2h/4   = {div}\");\n\n// 5. 字符串转换\nTimeSpan parsed = TimeSpan.Parse(\"02:30:00\");\nConsole.WriteLine($\"parsed = {parsed}\");\nTimeSpan.TryParse(\"01:30:00\", out TimeSpan tp);\nConsole.WriteLine($\"TryParse = {tp}\");\n\`\`\`\n\n### 五、DateTime 之差 ⭐⭐⭐\n\n\`\`\`csharp\nDateTime start = new DateTime(2026, 7, 18, 9, 0, 0);\nDateTime end = new DateTime(2026, 7, 18, 18, 30, 0);\n\nTimeSpan diff = end - start;\nConsole.WriteLine($\"差值 = {diff}\");\nConsole.WriteLine($\"小时 = {diff.TotalHours}\");\nConsole.WriteLine($\"天   = {diff.TotalDays}\");\nConsole.WriteLine($\"分钟 = {diff.TotalMinutes}\");\n\n// 实战：年龄计算\nDateTime birthday = new DateTime(1990, 5, 1);\nDateTime now = DateTime.Today;\nint age = now.Year - birthday.Year;\nif (now < birthday.AddYears(age)) age--; // 生日未到减 1\nConsole.WriteLine($\"年龄 = {age}\");\n\n// 实战：两个日期相隔几天\nint daysApart = (int)(end.Date - start.Date).TotalDays;\nConsole.WriteLine($\"相隔 {daysApart} 天\");\n\n// 实战：工作日（排除周末）\nint Workdays(DateTime from, DateTime to)\n{\n    int n = 0;\n    for (var d = from.Date; d <= to.Date; d = d.AddDays(1))\n    {\n        if (d.DayOfWeek != DayOfWeek.Saturday && d.DayOfWeek != DayOfWeek.Sunday)\n            n++;\n    }\n    return n;\n}\nConsole.WriteLine($\"工作日 = {Workdays(new DateTime(2026, 7, 1), new DateTime(2026, 7, 31))}\");\n\`\`\`\n\n### 六、格式化 ⭐⭐⭐\n\n\`\`\`csharp\nDateTime d = new DateTime(2026, 7, 18, 14, 35, 22);\n\n// 标准格式字符串\nConsole.WriteLine($\"d (短日期)     = {d:d}\");\nConsole.WriteLine($\"D (长日期)     = {d:D}\");\nConsole.WriteLine($\"t (短时间)     = {d:t}\");\nConsole.WriteLine($\"T (长时间)     = {d:T}\");\nConsole.WriteLine($\"f (完整短)     = {d:f}\");\nConsole.WriteLine($\"F (完整长)     = {d:F}\");\nConsole.WriteLine($\"g (常规短)     = {d:g}\");\nConsole.WriteLine($\"G (常规长)     = {d:G}\");\nConsole.WriteLine($\"s (排序)       = {d:s}\");  // ISO 8601\nConsole.WriteLine($\"u (UTC 排序)   = {d:u}\");\nConsole.WriteLine($\"O (往返)       = {d:O}\");  // 完整保留时区\n\n// 自定义格式\nConsole.WriteLine($\"yyyyMMdd       = {d:yyyyMMdd}\");\nConsole.WriteLine($\"yyyy-MM-dd     = {d:yyyy-MM-dd}\");\nConsole.WriteLine($\"HH:mm:ss       = {d:HH:mm:ss}\");\nConsole.WriteLine($\"yyyy年M月d日    = {d:yyyy年M月d日}\");\nConsole.WriteLine($\"hh:mm tt       = {d:hh:mm tt}\"); // 12-hour\nConsole.WriteLine($\"dddd           = {d:dddd}\");      // 星期名\nConsole.WriteLine($\"MMM            = {d:MMM}\");       // 月名缩写\n\`\`\`\n\n### 七、DateTime.Kind 与 UTC 转换 ⭐⭐\n\n\`\`\`csharp\nDateTime local = new DateTime(2026, 7, 18, 14, 30, 0, DateTimeKind.Local);\nDateTime utc = new DateTime(2026, 7, 18, 14, 30, 0, DateTimeKind.Utc);\nDateTime unspec = new DateTime(2026, 7, 18, 14, 30, 0);  // Unspecified\n\nConsole.WriteLine($\"local.Kind  = {local.Kind}\");\nConsole.WriteLine($\"utc.Kind    = {utc.Kind}\");\nConsole.WriteLine($\"unspec.Kind = {unspec.Kind}\");\n\n// ToUniversalTime / ToLocalTime\nDateTime localNow = DateTime.Now;\nDateTime utcNow = localNow.ToUniversalTime();\nDateTime back = utcNow.ToLocalTime();\nConsole.WriteLine($\"{localNow:HH:mm:ss} -> {utcNow:HH:mm:ss} -> {back:HH:mm:ss}\");\n\n// ⚠️ Unspecified 不会自动转换\nConsole.WriteLine($\"unspec.ToUniversalTime() = {unspec.ToUniversalTime()}\"); // 视为本地\n\`\`\`\n\n### 八、关键总结\n\n- \`DateTime.Now\` 本地时间，\`DateTime.UtcNow\` UTC（推荐存数据库）\n- 计算用 \`AddDays/AddHours/...\`：返回新对象（不可变）\n- \`TimeSpan\` 表示时间间隔：\`TotalHours\` vs \`Hours\` 区别\n- 格式化用 \`:d\`、\`:D\`、\`:O\`（往返）等\n- 解析用 \`TryParse\` 系列（异常安全）\n- 业务场景：年龄、工作日、月初月末、季度计算\n- 数据库存 UTC，显示转本地\n\n`,
  },

  // ============================================================
  // 第六十二章：DateTimeOffset、DateOnly、TimeOnly 与时区
  // ============================================================
  {
    id: 'csharp3-ch62',
    group: '第十二部分 日期时间与网络',
    icon: '🌍',
    title: '第六十二章 DateTimeOffset、DateOnly、TimeOnly 与时区',
    content: `## 第六十二章　DateTimeOffset、DateOnly、TimeOnly 与时区

本章讲解 .NET 6+ 的现代时间 API。\`DateTimeOffset\` 解决时区歧义，\`DateOnly\`/\`TimeOnly\` 解决"只有日期"和"只有时间"的场景。
...
\`\`\`\n\n### 八、关键总结\n\n- \`DateTime\`：含 Kind，容易丢时区\n- \`DateTimeOffset\`：始终带偏移量，跨时区推荐\n- \`DateOnly\`：纯日期（生日、节假日）\n- \`TimeOnly\`：纯时间（营业时间、闹钟）\n- \`TimeZoneInfo\`：时区转换、夏令时\n- 数据库：存 \`DateTime\` UTC + 业务时区字符串\n- 序列化：\`DateTimeOffset\` 往返更安全\n- .NET 6+ 优先用 \`DateOnly\`/\`TimeOnly\`\n\n**最佳实践**：\n1. 数据库：\`DateTime\` (UTC) + 业务时区 ID\n2. 业务展示：转本地时区\n3. API 响应：ISO 8601 带偏移量\n4. 用户输入：明确时区，不假设本地\n\n`,
  ...
  // ============================================================
  {
    ...
    group: '第十二部分 日期时间与网络',
    icon: '🔗',
    title: '第六十三章 Uri 与 URL 处理',
    content: `## 第六十三章　Uri 与 URL 处理

URI 是网络编程的基础。.NET 提供 \`System.Uri\` 类安全地解析、构造、操作 URL。
...
\`\`\`\n\n### 四、URL 编码 ⭐⭐⭐\n\n\`\`\`csharp\nusing System.Net;\n\nstring s = \"中文 & spaces?#\";\n\n// URL 编码（百分号编码）\nstring enc = Uri.EscapeDataString(s);\nConsole.WriteLine($\"EscapeDataString = {enc}\");\n// %E4%B8%AD%E6%96%87%20%26%20spaces%3F%23\n\n// 完整 URL 编码（保留 URL 结构字符）\nstring enc2 = Uri.EscapeUriString(s);\nConsole.WriteLine($\"EscapeUriString   = {enc2}\");\n// %E4%B8%AD%E6%96%87%20&%20spaces%3F%23 (& 不编码)\n\n// 解码\nstring dec = Uri.UnescapeDataString(enc);\nConsole.WriteLine($\"UnescapeDataString = {dec}\");\n\n// HTML 编码（用于 HTML 内容，不是 URL）\nstring htmlEnc = WebUtility.HtmlEncode(\"<a href=\\\"x\\\">test</a>\");\nConsole.WriteLine($\"HtmlEncode = {htmlEnc}\");\n\nstring htmlDec = WebUtility.HtmlDecode(htmlEnc);\nConsole.WriteLine($\"HtmlDecode = {htmlDec}\");\n\n// 选型：\n// URL 部分（如 query value）：Uri.EscapeDataString\n// 完整 URL：Uri.EscapeUriString\n// HTML 内容：WebUtility.HtmlEncode\n// 解码：对应方法\n\`\`\`\n\n### 五、UriBuilder：构造 URL ⭐⭐\n\n\`\`\`csharp\nvar b = new UriBuilder\n{\n    Scheme = \"https\",\n    Host = \"api.example.com\",\n    Port = 443,\n    Path = \"/v1/users/100\",\n    Query = \"include=profile&include=posts\"\n};\nConsole.WriteLine(b.Uri);\n\n// 增量添加 query\nb.Query = $\"{b.Query.TrimStart('?')}&token=abc123\";\nConsole.WriteLine(b.Uri);\n\n// 构造完整 URL\nstring url = new UriBuilder\n{\n    Scheme = \"https\",\n    Host = \"example.com\",\n    Path = \"/api\",\n    Fragment = \"results\"\n}.ToString();\nConsole.WriteLine(url);\n\n// 复制并修改\nUri source = new Uri(\"https://api.example.com/v1\");\nvar copy = new UriBuilder(source) { Path = \"/v2\" };\nConsole.WriteLine(copy.Uri);\n\`\`\`\n\n### 六、Uri 类型判断 ⭐\n\n\`\`\`csharp\nUri[] uris = {\n    new Uri(\"https://example.com\"),\n    new Uri(\"http://example.com\"),\n    new Uri(\"ftp://files.example.com/file.zip\"),\n    new Uri(\"mailto:user@example.com\"),\n    new Uri(\"file:///tmp/data.txt\"),\n    new Uri(\"https://example.com:8080/path\")\n};\n\nforeach (var u in uris)\n{\n    bool isHttps = u.Scheme == \"https\";\n    bool isHttp = u.Scheme == \"http\" || u.Scheme == \"https\";\n    bool isFile = u.IsFile;\n    bool isLoop = u.IsLoopback;\n    bool isUnc = u.IsUnc;\n    Console.WriteLine($\"{u,-40} isHttps={isHttps,-5} isFile={isFile,-5} loop={isLoop,-5}\");\n}\n\n// 绝对 vs 相对\nUri abs = new Uri(\"https://example.com/x\");\nUri rel = new Uri(\"x\", UriKind.Relative);\nConsole.WriteLine($\"abs.IsAbsoluteUri = {abs.IsAbsoluteUri}\");\nConsole.WriteLine($\"rel.IsAbsoluteUri = {rel.IsAbsoluteUri}\");\nConsole.WriteLine($\"rel.IsAbsoluteUri (rel) = {rel.IsAbsoluteUri}\");\n\n// Unc 路径（Windows）\ntry\n{\n    Uri unc = new Uri(@\"\\\\server\\share\\file.txt\");\n    Console.WriteLine($\"UNC = {unc}, IsUnc = {unc.IsUnc}\");\n}\ncatch { Console.WriteLine(\"UNC 仅 Windows\"); }\n\`\`\`\n\n### 七、文件路径与 Uri 互转 ⭐\n\n\`\`\`csharp\n// 文件路径 -> Uri\nstring path = Path.Combine(Environment.CurrentDirectory, \"data.txt\");\nUri fromPath = new Uri(path);\nConsole.WriteLine($\"fromPath = {fromPath}\");\nConsole.WriteLine($\"LocalPath = {fromPath.LocalPath}\");\n\n// Uri -> 文件路径\nUri fromUrl = new Uri(\"file:///tmp/data.txt\");\nstring back2 = fromUrl.LocalPath;\nConsole.WriteLine($\"back = {back2}\");\n\n// Windows 路径特殊处理\nif (OperatingSystem.IsWindows())\n{\n    Uri winPath = new Uri(\"C:\\\\Users\\\\alice\\\\file.txt\");\n    Console.WriteLine($\"winPath = {winPath}\");\n    Console.WriteLine($\"LocalPath = {winPath.LocalPath}\");\n}\n\`\`\`\n\n### 八、HttpClient 配合 Uri ⭐\n\n\`\`\`csharp\n// 基础用法（演示结构，不实际发请求）\nusing var http = new HttpClient { BaseAddress = new Uri(\"https://api.example.com/v1/\") };\n\n// BaseAddress 帮你自动拼接\nUri usersUri = new Uri(http.BaseAddress, \"users/100\");\nConsole.WriteLine($\"GET {usersUri}\");\n\nUri searchUri = new Uri(http.BaseAddress, \"search?q=hello\");\nConsole.WriteLine($\"GET {searchUri}\");\n\n// 完整路径覆盖\nUri fullUri = new Uri(\"https://other.com/path\");\nConsole.WriteLine($\"GET {fullUri}\");\n\`\`\`\n\n### 九、关键总结\n\n- \`Uri\`：安全解析 URL，自动处理编码\n- 构造 \`new Uri(baseUri, relative)\` 解析相对路径\n- 编码：\`Uri.EscapeDataString\`（用于 query value）\n- 编码：\`Uri.EscapeUriString\`（用于完整 URL）\n- \`HttpUtility.ParseQueryString\`：解析 query string\n- \`UriBuilder\`：可变构造 URL\n- \`WebUtility\`：HTML/URL 通用编解码\n- \`IsFile\`、\`IsLoopback\`、\`IsUnc\`：Uri 类型判断\n- 文件路径和 Uri 互转注意 Windows 盘符\n\n`,
  ...
  // ============================================================
  {
    ...
    group: '第十二部分 日期时间与网络',
    icon: '🌐',
    title: '第六十四章 HttpClient 基础',
    content: `## 第六十四章　HttpClient 基础

\`HttpClient\` 是 .NET 中发 HTTP 请求的现代 API。从 .NET 5 起官方推荐 \`HttpClient\` 替代 \`WebClient\`。
...
\`\`\`\n\n### 六、其他方法 ⭐⭐\n\n\`\`\`csharp\nusing var client = new HttpClient { BaseAddress = new Uri(\"https://api.example.com/\") };\n\n// PUT / PATCH / DELETE\n// await client.PutAsJsonAsync(\"users/100\", new { name = \"new\" });\n// await client.PatchAsJsonAsync(\"users/100\", new { age = 31 });\n// await client.DeleteAsync(\"users/100\");\n\n// HEAD（只取头，不取 body）\n// HttpResponseMessage resp = await client.SendAsync(new HttpRequestMessage(HttpMethod.Head, \"users\"));\n// Console.WriteLine(resp.Headers);\n\nConsole.WriteLine(\"PUT/PATCH/DELETE/HEAD 都有对应方法\");\n\`\`\`\n\n### 七、添加自定义头 ⭐⭐\n\n\`\`\`csharp\n// 单次请求头\nusing var req = new HttpRequestMessage(HttpMethod.Get, \"users/100\");\nreq.Headers.Add(\"Authorization\", $\"Bearer {Guid.NewGuid()}\");\nreq.Headers.Add(\"X-Request-Id\", Guid.NewGuid().ToString());\n\nConsole.WriteLine($\"Authorization: {req.Headers.Authorization}\");\nConsole.WriteLine($\"X-Request-Id: {req.Headers.GetValues(\"X-Request-Id\").First()}\");\n\n// 实际发送：\n// using var client = new HttpClient();\n// using var resp = await client.SendAsync(req);\n\n// 用 SendAsync 替代 GetAsync\nConsole.WriteLine(\"SendAsync 用于自定义头\");\n\`\`\`\n\n### 八、错误处理 ⭐⭐\n\n\`\`\`csharp\nusing var client = new HttpClient();\n\nasync Task<string> SafeGet(string url)\n{\n    try\n    {\n        // 实际：return await client.GetStringAsync(url);\n        throw new HttpRequestException(\"网络失败（演示）\");\n    }\n    catch (HttpRequestException ex)\n    {\n        Console.WriteLine($\"网络错误: {ex.Message}\");\n        return string.Empty;\n    }\n    catch (TaskCanceledException ex) when (ex.InnerException is TimeoutException)\n    {\n        Console.WriteLine($\"超时: {ex.Message}\");\n        return string.Empty;\n    }\n}\n\n// 模拟调用\n// string r = await SafeGet(\"https://example.com\");\nConsole.WriteLine(\"异常类型：HttpRequestException, TaskCanceledException\");\n\n// 状态码检查\n// HttpResponseMessage resp = await client.GetAsync(url);\n// if (resp.IsSuccessStatusCode) { ... } else { ... }\n// resp.EnsureSuccessStatusCode();\nConsole.WriteLine(\"EnsureSuccessStatusCode 2xx 之外抛异常\");\n\`\`\`\n\n### 九、取消令牌 ⭐\n\n\`\`\`csharp\nusing var client = new HttpClient();\nusing var cts = new CancellationTokenSource();\ncts.CancelAfter(TimeSpan.FromSeconds(5));\n\ntry\n{\n    // 实际：await client.GetAsync(\"https://example.com\", cts.Token);\n    Console.WriteLine(\"5 秒后自动取消\");\n}\ncatch (TaskCanceledException)\n{\n    Console.WriteLine(\"请求被取消\");\n}\n\nConsole.WriteLine(\"支持取消令牌\");\n\`\`\`\n\n### 十、关键总结\n\n| 方法 | 用途 |\n| --- | --- |\n| \`GetStringAsync\` | 拿响应字符串 |\n| \`GetByteArrayAsync\` | 拿响应字节 |\n| \`GetStreamAsync\` | 拿响应流 |\n| \`GetAsync\` | 拿完整响应 |\n| \`PostAsync\` | 发送任意内容 |\n| \`PostAsJsonAsync\` | 发送 JSON |\n| \`SendAsync\` | 完全控制请求 |\n| \`DeleteAsync\` | 删除 |\n| \`PutAsync\` / \`PatchAsync\` | 更新 |\n\n- **不要**频繁 \`new HttpClient()\`：会泄漏 socket\n- **推荐**：\`IHttpClientFactory\` 注入\n- 配置：\`BaseAddress\`、\`Timeout\`、\`DefaultRequestHeaders\`\n- 错误处理：\`HttpRequestException\`、\`TaskCanceledException\`\n- 实际项目用 \`Microsoft.Extensions.Http\` 扩展包\n\n`,
  ...
  // ============================================================
  // 第六十五章：HttpClient 进阶（JSON、Header、重试）
  // ============================================================
  {
    id: 'csharp3-ch65',
    group: '第十二部分 日期时间与网络',
    icon: '🚀',
    title: '第六十五章 HttpClient 进阶（JSON、Header、重试）',
    content: `## 第六十五章　HttpClient 进阶（JSON、Header、重试）

本章讲解 HttpClient 的进阶用法：JSON 序列化、Header 管理、重试、超时、下载进度等。
...
\`\`\`\n\n### 九、关键总结\n\n- \`PostAsJsonAsync\` / \`ReadFromJsonAsync\`：自动 JSON 序列化\n- \`DelegatingHandler\`：拦截器（日志、认证、重试）\n- \`Polly\`：重试、熔断、超时、降级\n- \`HttpCompletionOption.ResponseHeadersRead\`：流式下载\n- \`IProgress<T>\` + \`ReadAsync\`：下载进度\n- **不要**频繁 new HttpClient：用 IHttpClientFactory 或静态单例\n- \`PooledConnectionLifetime\`：避免 DNS 缓存问题\n- 测试用 \`HttpMessageHandler\` 模拟\n\n**HttpClient 三种使用模式**：\n1. \`IHttpClientFactory\`（DI 注入）⭐⭐⭐ 推荐\n2. 静态单例 + SocketsHttpHandler\n3. 短生命周期 using（次优）\n\n`,
  ...
  // ============================================================
  // 第六十六章：Socket 与 TCP/UDP 编程
  // ============================================================
  {
    id: 'csharp3-ch66',
    group: '第十二部分 日期时间与网络',
    icon: '🛰️',
    title: '第六十六章 Socket 与 TCP/UDP 编程',
    content: `## 第六十六章　Socket 与 TCP/UDP 编程

Socket 是网络编程的底层 API。本章讲解 TCP/UDP Socket 的基础用法（生产中多用 HttpClient/Kestrel，但理解 Socket 有助于理解底层）。
