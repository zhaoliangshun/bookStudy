...
// ---------- 可执行代码 ----------
var p = new Printer();
p.Print("hello");          // 编译器根据参数类型选重载 1（string 版本）
p.Print(42);               // 选重载 2（int 版本）
p.Print("分数", 95);       // 选重载 3（string, int）——参数精确匹配
p.Print("hi", (byte)3);    // 选重载 4（string, byte）；不传第二个参数时 times 取默认值 1
...
class Printer {
    ...
    // ⚠️ 小心：可选参数让一个重载"能吃下"多种调用，容易和别的重载抢。
    // 这里特意把第二个参数写成 byte 而不是 int：写成 int 就和重载 3 撞签名 (string, int)，
    // 编译器直接报 CS0111「已定义具有相同参数类型的成员」，连编译都过不去。
    public void Print(string s, byte times = 1) {
...
> ⭐ 方法重载让 API 简洁——一个 \`Print\` 名字搞定多种输入。但**可选参数 + 重载容易撞车**：两个重载的签名不能长得一样（否则 CS0111），而且调用时编译器挑"最匹配"的那个，结果未必是你直觉想要的。要么别混用，要么像上面那样让签名明确区分开。
...
dict[m1] = "第一笔";
// 注意：插值字符串里再嵌双引号很容易把字符串截断（CS8076），先算好再插值更稳
string found = dict.TryGetValue(m2, out var v) ? v : "无";
Console.WriteLine($"查 m2：{found}");  // 第一笔
