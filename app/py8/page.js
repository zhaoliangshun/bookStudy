...
// =============================================================
// Python 全而大教程（py8）交互式页面
// -------------------------------------------------------------
// 全新制作，覆盖 100% 日常开发知识，每知识点配详细 demo
//   1. 数据源：py8Chapters / py8ChapterGroups
...
export default function Py8Tutorial() {
  // 从 URL hash 读取初始章节 id，如果没有则使用第一个章节
  const getInitialChapterId = () => {
    if (typeof window !== "undefined") {
      const hash = window.location.hash.slice(1);
      if (hash && py8Chapters.find((c) => c.id === hash)) {
        return hash;
      }
    }
    return py8Chapters[0].id;
  };

  const initialId = getInitialChapterId();
  const initialChapter = py8Chapters.find((c) => c.id === initialId) || py8Chapters[0];
  ...
  const activeChapter =
    py8Chapters.find((c) => c.id === activeId) || py8Chapters[0];

  const selectChapter = useCallback((chapterId) => {
    const chapter = py8Chapters.find((c) => c.id === chapterId);
    if (!chapter) return;
  ...
  const groupedChapters = py8ChapterGroups.map((group) => ({
    group,
    items: py8Chapters.filter((c) => c.group === group),
  }));
    ...
    <div className="app-shell">
      <div className="main-layout">
        <Sidebar
          ...
          currentPath="/py8"
          meta={`共 ${py8Chapters.length} 章 · 全新详解版`}
        />
        ...
        <main className="content" ref={contentRef}>
          ...
          <footer className="content-footer">
            <p>
              Python 全而大教程（py8）· 代码由系统 python3 子进程执行 · 覆盖日常开发 100% 知识点
            </p>
          </footer>
        </main>
      </div>
...
function ChapterNav({ activeId, onSelect }) {
  const idx = py8Chapters.findIndex((c) => c.id === activeId);
  const prev = idx > 0 ? py8Chapters[idx - 1] : null;
  const next = idx < py8Chapters.length - 1 ? py8Chapters[idx + 1] : null;
