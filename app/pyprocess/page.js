...
// =============================================================
// Python 多进程教程（pyprocess）交互式页面
// -------------------------------------------------------------
// 专题教程：专注讲解 multiprocessing 多进程的工作原理与日常开发应用
//   1. 数据源：pyprocessChapters / pyprocessChapterGroups（来自 pyprocess-tutorial-data）
...
export default function PyProcessTutorial() {
  // 默认使用第一个章节作为初始状态。
  ...
  const initialChapter = pyprocessChapters[0];
  ...
  // 当前章节对象
  const activeChapter =
    pyprocessChapters.find((c) => c.id === activeId) || pyprocessChapters[0];
  ...
  useEffect(() => {
    ...
    if (!hash) return;
    const chapter = pyprocessChapters.find((c) => c.id === hash);
    if (chapter) {
  ...
  const selectChapter = useCallback((chapterId) => {
    const chapter = pyprocessChapters.find((c) => c.id === chapterId);
    if (!chapter) return;
  ...
  // 按分组组织章节
  const groupedChapters = pyprocessChapterGroups.map((group) => ({
    group,
    items: pyprocessChapters.filter((c) => c.group === group),
  }));
    ...
    <div className="app-shell">
      <div className="main-layout">
        ...
        <Sidebar
          ...
          currentPath="/pyprocess"
          meta={`共 ${pyprocessChapters.length} 章 · 多进程原理 + 实战 demo`}
        />
        ...
        <main className="content" ref={contentRef}>
          ...
          <footer className="content-footer">
            <p>
              Python 多进程教程 · 代码由系统 python3 子进程执行 · 简单易懂讲原理，demo 驱动学实战
            </p>
          </footer>
        </main>
      </div>
...
function ChapterNav({ activeId, onSelect }) {
  const idx = pyprocessChapters.findIndex((c) => c.id === activeId);
  const prev = idx > 0 ? pyprocessChapters[idx - 1] : null;
  const next = idx < pyprocessChapters.length - 1 ? pyprocessChapters[idx + 1] : null;
