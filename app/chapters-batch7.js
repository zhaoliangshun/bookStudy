...
**MySQL 协议与连接握手**：客户端与 MySQL 服务器通信基于自定义的 TCP 协议，一次完整的连接建立包含以下步骤：
...
- 握手阶段使用挑战-响应认证：服务器下发随机盐，客户端用 SHA1 加密密码后回传，避免明文传输
- 每条 SQL 走一次"发送→解析→优化→执行→返回"流程；预编译语句（Prepared Statement）可省去前两步的重复开销
- 索引底层多采用 **B+ 树**：非叶子节点只存索引键，叶子节点用链表相连，范围查询可顺序遍历：

\`\`\`
           [10 | 20 | 30]            ← 根节点（非叶子）
          /      |       \\
   [1,5,8]   [10,12,15]   [20,25,28] [30,35]  ← 叶子节点（含数据指针）
      ↔          ↔            ↔         ↔    ← 双向链表
\`\`\`

### 8. 「常见陷阱」：MySQL 集成的 4 个坑

**① 字符串拼接 SQL 导致注入**：
\`\`\`javascript
// ❌ 错误：用户输入直接拼接，可被 OR '1'='1 绕过
...
**② N+1 查询**：循环里执行单条查询，导致 1 次列表查询 + N 次详情查询。
\`\`\`javascript
// ❌ 错误
const users = await conn.query('SELECT id FROM users');
for (const u of users) {
  const orders = await conn.query('SELECT * FROM orders WHERE user_id = ?', [u.id]);
}

// ✅ 正确：一次性用 IN 或 JOIN
const ids = users.map(u => u.id);
const orders = await conn.query('SELECT * FROM orders WHERE user_id IN (?)', [ids]);
\`\`\`

**③ 忘记释放连接**：手动 acquire 后异常未走 finally，连接泄漏。
\`\`\`javascript
// ❌ 错误
const conn = await pool.acquire();
const rows = await conn.query('SELECT ...'); // 一旦抛错，连接永远不归还
...
**④ 事务范围过大**：在事务里包了 RPC、HTTP 请求，导致锁长持有、连接占用过久。
\`\`\`javascript
// ❌ 错误：事务里调用外部接口
const t = await conn.beginTransaction();
try {
  await conn.update(...);
  await axios.post('/notify'); // 网络抖动会让事务挂死
  await t.commit();
} catch { await t.rollback(); }

// ✅ 正确：事务只包 DB 操作，外部调用放在事务外
await t.commit();
await axios.post('/notify');
\`\`\`

### 9. 「性能提示」：MySQL 调优要点

1. **索引设计遵循"最左前缀 + 高选择性"**：联合索引 (a,b,c) 只能命中 a / a,b / a,b,c；将区分度高的列放前面。避免在索引列上使用函数或隐式类型转换，否则索引失效。
2. **批量插入代替循环单条插入**：\`INSERT INTO t VALUES (?,?,?), (?,?,?), ...\` 比循环 N 次单条 INSERT 快 10 倍以上，可显著减少网络往返和事务开销。
3. **连接池大小不是越大越好**：经验公式 \`pool_size = (核心数 * 2 + 磁盘数)\` 对 CPU 密集型有效；IO 密集型可适当放大，但过大反而会增加数据库上下文切换开销。建议从 10 起步压测调优。
`,
    code: `// =============================================================
...
### 7. 「底层原理」：MongoDB 通信与存储

**Wire Protocol**：MongoDB 客户端与服务端通过 TCP 长连接 + 自定义二进制协议通信，默认端口 27017。所有请求以 **OP_MSG** 消息（4.x 起统一）封装：

\`\`\`
+----------+--------+----------+--------------+
| MsgHeader|flagBits| sections | checksum(可选)|
| (16字节) | (4字节)| (变长)    | (4字节)      |
+----------+--------+----------+--------------+
   ↓ MsgHeader 包含 messageLength / requestID / responseTo / opCode
\`\`\`

- 每个请求带 requestID，服务端响应带 responseTo 与之配对，支持**单连接上的请求管线化**
- **BSON** 在 JSON 基础上扩展：类型字段+长度前缀，支持 Date/BinData/ObjectId/Decimal128 等，遍历可跳过未知字段
- **ObjectId 结构**（12 字节）：4 字节时间戳 + 5 字节随机值 + 3 字节递增计数器，可近似按插入时间排序
...
- 存储引擎 **WiredTiger** 默认使用 B+ 树 + **文档级 MVCC**，写时通过 snapshot 实现事务隔离，3.0 之后取代了 MMAPv1。

### 8. 「常见陷阱」：MongoDB 集成的 4 个坑

**① 文档大小超过 16MB 限制**：
\`\`\`javascript
// ❌ 错误：把评论数组无限往文档里塞
db.posts.updateOne({ _id: 1 }, { $push: { comments: newComment } });
// comments 膨胀到 16MB 后写入直接报错

// ✅ 正确：评论量大时单独建集合，用引用关联
db.comments.insertOne({ postId: 1, ...newComment });
\`\`\`

**② 查询未命中索引导致全表扫描**：
\`\`\`javascript
// ❌ 错误：对 {a:1, b:2} 建索引，却以 b 开头查
db.t.find({ b: 2 });                       // 不命中联合索引
db.t.find({ a: 1, 'arr.0': x });           // 多键索引无法精准匹配数组位置

// ✅ 正确：按 ESR 法则建索引（Equality → Sort → Range）
db.t.createIndex({ b: 1 });                // 单独为 b 建索引
db.t.createIndex({ status: 1, createdAt: -1, amount: 1 });
\`\`\`

**③ 把关系型思维搬过来，过度用 $lookup 做 JOIN**：
\`\`\`javascript
// ❌ 错误：聚合管道里多次 $lookup，性能远低于嵌入文档
db.users.aggregate([
  { $lookup: { from: 'orders', localField: '_id', foreignField: 'userId', as: 'orders' } },
  { $lookup: { from: 'items',  localField: 'orders.itemId', foreignField: '_id', as: 'items' } }
]);

// ✅ 正确：高频一起读的数据直接嵌入；只有低频、大体积才引用
db.users.insertOne({ profile: {...}, recentOrders: [{...}, {...}] });
\`\`\`

**④ 大量写入未走批量接口**：
\`\`\`javascript
// ❌ 错误：循环 insertOne
for (const doc of docs) await db.t.insertOne(doc);

// ✅ 正确：insertMany + ordered:false（无序并行写入更快）
await db.t.insertMany(docs, { ordered: false });
\`\`\`

### 9. 「性能提示」：MongoDB 调优要点

1. **索引按 ESR 法则设计**：等值条件列在前、排序字段其次、范围查询列最后；用 \`explain('executionStats')\` 验证 \`totalDocsExamined\` 接近 \`nReturned\`，避免 COLLSCAN。
2. **读偏好与写关注按场景调整**：写敏感场景用 \`w:1\` 换吞吐；强一致场景用 \`w:'majority'\`；从库分担读流量时用 \`readPreference='secondary'\` 但要容忍最终一致。
3. **批量写 + 无序模式**：\`insertMany(docs, { ordered: false })\` 让 MongoDB 并行写入，单条失败不阻断其他；配合 \`writeConcern: { w: 0 }\` 可在日志类场景进一步提升吞吐（牺牲确认）。
`,
    code: `// =============================================================
...
**LIFO vs FIFO 复用策略**：连接池在归还连接时有两种典型策略，影响连接的实际生命周期。

\`\`\`
LIFO (后进先出)            FIFO (先进先出)
[free] A B C  ← 归还       [free] A B C  ← 归还
[free] C B A  ← 借出       [free] A B C  ← 借出
↑ 反复复用最近刚用的连接     ↑ 每个连接都有机会被用
  优点：keepalive 命中率高    优点：连接负载均匀
        冷连接尽快被淘汰      缺点：可能命中已 idle 的连接
\`\`\`

- **LIFO**（如 HikariCP、generic-pool 默认）：刚归还的连接立刻被再次借出，TCP keepalive 持续有效，空闲连接自然沉底被回收
- **FIFO**：负载更均匀，但每个连接都可能闲置较久，需要更频繁的 ping 健康检查

**keepalive 与连接保活**：
\`\`\`
应用                          数据库
  |  连接 #5 (空闲 50s)             |
  |  无业务流量                     |  ← 防火墙/NAT 可能丢弃空闲连接
  |  TCP keepalive 探测包 ────────> |
  |  <───── ACK                    |
  |  连接保持有效                   |
\`\`\`

- OS 层 \`tcp_keepalive\` 默认 7200s 太长，通常需通过 \`socket.setTimeout()\` 或应用层 ping 缩短到 30s~60s
- 连接池的 \`maxLifetime\` 强制重建超龄连接，规避长连接内存泄漏（如 MySQL 的 \`wait_timeout\` 主动断开）

### 8. 「常见陷阱」：连接池的 4 个坑

**① 异常路径下连接未归还**：
\`\`\`javascript
// ❌ 错误：提前 return 或抛错跳过 release
async function getUser(id) {
  const conn = await pool.acquire();
  const row = await conn.query('SELECT ...');
  if (!row) return null;          // ← 连接泄漏！
  pool.release(conn);
  return row;
}
...
\`\`\`javascript
// ❌ 错误：以为连接越多越快
new Pool({ max: 500 });           // MySQL 默认 151 个连接，应用直接打爆

// ✅ 正确：参考 PostgreSQL 公式 pool_size = (core*2 + effective_spindle_count)
new Pool({ max: 20 });            // 中小应用通常 10~30 足够
\`\`\`

**③ 不做健康检查，拿到的是死连接**：
\`\`\`javascript
// ❌ 错误：直接用 acquire 拿到的连接
const conn = await pool.acquire();
await conn.query('SELECT 1');     // MySQL 重启后报 EPIPE

// ✅ 正确：开启池的 healthCheck + 借出前 ping
pool.on('acquire', conn => conn.ping().catch(() => pool.destroy(conn)));
// 或借助库自带的 testOnBorrow 选项
\`\`\`

**④ 等待队列无超时，请求堆积雪崩**：
\`\`\`javascript
// ❌ 错误：acquireTimeout 设为 0（无限等）
new Pool({ acquireTimeout: 0 });  // 慢 SQL 把池占满后所有请求挂死

// ✅ 正确：设置合理超时 + 上游限流
new Pool({ acquireTimeout: 3000 });
// 超时后抛错，由网关返回 503，触发熔断保护
\`\`\`

### 9. 「性能提示」：连接池调优要点

1. **池大小从 \`2*CPU + 磁盘数\` 起步压测**：CPU/IO 密集型任务对并发连接数有上限，超过后上下文切换成本 > 并行收益。HikariCP 官方建议 MySQL/PG 在 10~30 区间，不要超过数据库 \`max_connections / 应用实例数\`。
2. **配置 \`min\` 维持热连接 + \`maxLifetime\` 防泄漏**：min 设 2~5 避免冷启动握手开销；maxLifetime 设 30min~1h 强制重建，规避 MySQL \`wait_timeout\` 主动断开、内存碎片累积等问题。
3. **监控四要素：\`used / free / waiting / waitTimeouts\`**：等待队列长度 > 0 说明池已饱和；waitTimeouts 持续增长说明 SQL 太慢或池太小，应优先优化慢查询而非加池大小。
`,
    code: `// =============================================================
// 连接池管理器 —— 模拟数据库连接池
...
### 8. 「底层原理」：ORM 如何把对象变成 SQL

**查询构建器的 AST 转换**：链式调用并不是直接拼接字符串，而是先构造一棵抽象语法树（AST），最后由"方言层"翻译成目标数据库的 SQL。

\`\`\`
代码                          AST                      SQL
db('users')                  SelectStmt               SELECT name,email
 .select('name','email')   ┌─┤ table: users            FROM users
 .where('age','>',18)      │ ├─ projection: [name,email] WHERE age > 18
 .orderBy('age','asc')     │ ├─ where: > (age, 18)     ORDER BY age ASC
 .limit(10)                │ ├─ orderBy: (age, ASC)    LIMIT 10
                           │ └─ limit: 10
                           │
   方言层                   │  MySQL 方言: ? 占位
   ↓                       │  PG 方言:   $1 占位
...
- AST 中间表示让 ORM 可以做**SQL 重写优化**：自动加 LIMIT、把 N+1 改写成 IN 查询、补全 RETURNING 子句
- **Active Record vs Data Mapper** 的本质差异：方法挂在模型实例上还是独立的 Repository 上，前者方便后者解耦

**模型 → 表的元数据映射**：
\`\`\`
class User  ←─映射─→  table: users
  name: STRING        column: name VARCHAR(255)
  age:  INTEGER       column: age  INT
  orders: hasMany     外键 orders.user_id → users.id
\`\`\`

- ORM 启动时扫描装饰器/字段定义，构建元数据表，运行时根据元数据生成 SQL、做类型转换与校验

### 9. 「常见陷阱」：ORM 的 4 个坑

**① N+1 查询**：访问关联属性时每行触发一次 SQL。
\`\`\`javascript
// ❌ 错误：循环里访问关联
const users = await User.findAll();
for (const u of users) {
  console.log(u.orders.length);   // 每个用户一次 SQL，N 个用户 = N+1 次
}

// ✅ 正确：用 eager loading 一次性 JOIN/IN
const users = await User.findAll({ include: [{ model: Order }] });
\`\`\`

**② 隐式事务 + 大事务**：ORM 默认每条 save() 自动开事务，循环 save 等于 N 个事务。
\`\`\`javascript
// ❌ 错误
for (const u of users) await u.save();   // 100 个事务，慢且不一致

// ✅ 正确：显式包一个大事务
await sequelize.transaction(async t => {
  for (const u of users) await u.save({ transaction: t });
});
\`\`\`

**③ SELECT * 拖慢查询**：ORM 默认取所有字段，包括大 TEXT/BLOB。
\`\`\`javascript
// ❌ 错误
const users = await User.findAll();              // 取出 avatar (MEDIUMBLOB)
...
**④ 在 ORM 里写复杂查询，可读性与性能双输**：
\`\`\`javascript
// ❌ 错误：用 ORM 拼窗口函数/递归 CTE，代码比 SQL 还长
User.findAll({
  include: [{ include: [{ include: [...] }]}],
  group: [...], having: ...
});

// ✅ 正确：复杂查询直接用原生 SQL / 查询构建器
const rows = await sequelize.query(
  'WITH RECURSIVE ... SELECT ...', { type: QueryTypes.SELECT }
);
\`\`\`

### 10. 「性能提示」：ORM 调优要点

1. **默认开启 eager loading，按需 lazy**：列表场景提前 \`include\` 关联避免 N+1；详情页才按需懒加载，避免一次性取过宽的关联树。
2. **批量操作走原生接口**：\`Model.bulkCreate([...])\` 比 N 次 \`save()\` 快一个数量级；大批量导入直接用 \`LOAD DATA INFILE\` 或数据库原生 COPY，绕过 ORM 的钩子与校验。
3. **关闭生产环境的 SQL 日志，改用慢查询采样**：ORM 默认 \`logging: console.log\` 会同步打印每条 SQL，高 QPS 下 IO 开销显著；用 \`benchmark: true\` 只记录 >50ms 的慢查询。
`,
    code: `// =============================================================
...
### 8. 「底层原理」：事务的 ACID 与锁机制

**ACID 如何实现**：

| 特性 | 实现机制 |
|------|---------|
| 原子性 A | **Undo Log**：操作前写反向日志，回滚时按日志"撤销" |
| 持久性 D | **Redo Log**：操作后先写 WAL，崩溃后重放恢复 |
| 隔离性 I | **锁 + MVCC**：写写靠锁串行，读写靠多版本快照 |
| 一致性 C | A+I+D + 应用约束共同保证 |
...
事务执行流程：
  BEGIN
    ├─ 写 Undo Log (旧值)            ← 支持回滚
    ├─ 修改内存缓冲池
    ├─ 写 Redo Log (新值, 顺序IO)    ← 支持崩溃恢复
    └─ COMMIT (Redo Log fsync)       ← 提交点
\`\`\`

**MVCC 多版本并发控制**（InnoDB 的 REPEATABLE READ）：
\`\`\`
           事务A (读)              事务B (写)
T1  ── 开始事务 (快照 S1) ──
...
T5       SELECT balance  ←  读到的是 S1 快照中的旧值（不阻塞）
T6                            COMMIT;  ← 新值生效
T7  ── COMMIT ──            ← A 全程看到旧值，隔离性保证
\`\`\`

- 每行在底层存 \`trx_id\`（最后修改它的事务ID），读事务只看 \`trx_id <= 自己快照\` 的版本，写事务更新产生新版本
- 隔离级别本质是"快照的可见性规则"：READ COMMITTED 每条 SQL 重新快照；REPEATABLE READ 整个事务用同一快照
...
REPEATABLE READ   ✗          ✗         ✓*    ← MySQL 默认 (*RR靠间隙锁可避免幻读)
SERIALIZABLE      ✗          ✗         ✗     ← 性能差，很少用
\`\`\`
...
try {
  ...
  await axios.post('/notify');     // 网络抖动 → 事务悬挂几十秒，锁不释放
  await t.commit();
} catch { await t.rollback(); }
...
await axios.post('/notify');        // 事务外发送
\`\`\`

**② 忘记回滚，连接被卡住**：
\`\`\`javascript
// ❌ 错误：catch 里没 rollback
const t = await sequelize.transaction();
await Account.update(..., { transaction: t });
throw new Error('boom');  // 事务未回滚，连接持有的锁永远不释放

// ✅ 正确：try/catch 必回滚，或用自动回滚的回调式
try {
  await sequelize.transaction(async t => {
    await Account.update(..., { transaction: t });
...
**③ 长事务拖死整库**：事务里循环处理百万行，锁持有时间过长。
\`\`\`javascript
// ❌ 错误：一个事务处理全表
await sequelize.transaction(async t => {
  const users = await User.findAll();       // 100w 行
  for (const u of users) {
...
for (let i = 0; i < ids.length; i += SIZE) {
  await sequelize.transaction(async t => {
    await User.update(..., { where: { id: ids.slice(i, i+SIZE) }, transaction: t });
  });
}
\`\`\`
...
**④ 隔离级别选错，要么不一致要么性能差**：
\`\`\`javascript
// ❌ 错误：所有事务都用 SERIALIZABLE，吞吐骤降
sequelize.transaction({ isolationLevel: SERIALIZABLE }, async t => {...});

// ✅ 正确：按场景选
// - 报表统计：READ UNCOMMITTED/READ COMMITTED 即可
// - 转账：REPEATABLE READ（默认）
// - 唯一约束强校验：SERIALIZABLE 或乐观锁
\`\`\`

### 10. 「性能提示」：事务调优要点

1. **事务尽量短小**：把计算、IO、外部调用移出事务边界，事务只包"必要的写操作"。短事务减少锁持有时间，降低死锁概率，提升并发吞吐。规则：单事务 < 50ms 是健康的，> 500ms 必须优化。
2. **按场景选乐观锁 vs 悲观锁**：冲突少（如用户编辑自己的资料）用乐观锁（version 字段 + UPDATE WHERE version=?），无锁等待、吞吐高；冲突多（如秒杀库存）用悲观锁 \`SELECT ... FOR UPDATE\`，避免大量乐观锁重试。
3. **批量提交 + 合理隔离级别**：把 N 条单行 update 合并成一条 \`UPDATE ... WHERE id IN (...)\`，再配合默认的 REPEATABLE READ 即可在一致性与性能间取得平衡；只有必要时才升级到 SERIALIZABLE。
`,
    code: `// =============================================================
...
**LRU（Least Recently Used）** — 双向链表 + 哈希表，O(1) 访问与淘汰：

\`\`\`
哈希表 (key→Node)              双向链表（按最近访问排序）
  "a" ─┐                      head ⇄ [a|t=3] ⇄ [c|t=2] ⇄ [b|t=1] ⇄ tail
  "b" ─┼──> Node               ↑ 最近使用                最久未使用 ↑
  "c" ─┘                       访问 a → 把 a 移到 head 后
                              插入 d → 淘汰 tail 前的 b
\`\`\`

**LFU（Least Frequently Used）** — 按访问次数淘汰，保留高频热点：

\`\`\`
频次链:
  freq=1: [x]            ← 淘汰候选
  freq=2: [y]
  freq=5: [a, b]         ← 热点，几乎不被淘汰

访问 a → a.freq: 5→6，从 freq=5 链移到 freq=6 链
容量满 → 取 min_freq 链的 LRU 节点淘汰
\`\`\`

- LRU 适合**时间局部性强**的场景（最近访问的还会再访问），如用户会话、新闻流
- LFU 适合**热点稳定**的场景（少数 key 长期高频），但新 key 容易被淘汰，需要配"老化"机制

**缓存与数据库一致性**（Cache-Aside 的两种写策略）：

\`\`\`
策略A：先更新DB，再删缓存（推荐）
  UPDATE DB ──> DELETE cache
  下次读会触发 cache-aside 重新加载
  并发风险：小（仅极短窗口可能读到旧值）

策略B：先删缓存，再更新DB（陷阱多）
  DELETE cache ──> UPDATE DB
  并发风险：另一个读请求在 DB 更新前回填旧值，缓存长期不一致
\`\`\`
...
// ❌ 错误
async function getUser(id) {
  const v = cache.get('user:' + id);
  if (v) return v;
  const row = await db.find(id);
  if (row) cache.set('user:' + id, row);
  return row;                    // 不存在的 id 每次都查 DB
}

// ✅ 正确：空值也缓存（短 TTL），或前置布隆过滤器
if (row) cache.set(key, row);
else cache.set(key, null, 60000);  // 空值缓存 60s
\`\`\`

**② 缓存击穿**：热点 key 过期瞬间，海量请求同时打 DB。
\`\`\`javascript
// ❌ 错误：所有请求都查 DB
const v = cache.get(key);
if (!v) return await db.find(id);   // 1w 请求同时 miss
...
if (lock) {
  ...
  await sleep(50); return cache.get(key); // 其他人短暂等待
}
\`\`\`
...
\`\`\`javascript
// ❌ 错误：所有缓存用相同 TTL
for (const k of keys) cache.set(k, value, 300000);

// ✅ 正确：TTL 加随机抖动
for (const k of keys) cache.set(k, value, 300000 + Math.random() * 60000);
\`\`\`

**④ 缓存键设计冲突**：不同业务用了同样的 key 前缀。
\`\`\`javascript
// ❌ 错误：无命名空间，user:1 可能被 order 模块覆盖
cache.set('1', user);
cache.set('1', order);   // 互相覆盖

// ✅ 正确：业务模块:实体:ID 的层级命名
cache.set('user:profile:1', user);
cache.set('order:detail:1', order);
\`\`\`

**⑤ 大对象缓存导致内存膨胀与序列化开销**：
\`\`\`javascript
// ❌ 错误：把整张表缓存为一个大 JSON
cache.set('all_users', JSON.stringify(users));   // 50MB，每次读写都序列化

// ✅ 正确：分片缓存 + 用 Hash 结构存字段
cache.set('user:' + u.id, u);                      // 单条缓存
redis.hset('user:' + id, 'name', name, 'age', age); // Hash 按字段读写
\`\`\`

### 9. 「性能提示」：缓存调优要点

1. **多级缓存 + TTL 随机化**：本地 LRU（< 1ms）兜底 + Redis（< 1ms）作主缓存 + DB 兜底；每层 TTL 加 ±20% 随机抖动，避免雪崩。本地缓存容量按 \`热点 key 数 * 单条大小\` 估算，通常 1000~10000 条够用。
2. **选 LRU 还是 LFU 看访问模式**：时间局部性强（最近访问的更可能再访问，如用户会话、新闻流）选 LRU；热点稳定且少量 key 高频访问（如排行榜、配置）选 LFU。Redis 4.0+ 支持 \`maxmemory-policy allkeys-lfu\`。
3. **监控命中率而非缓存大小**：命中率 < 90% 说明缓存基本无效，应检查 TTL 是否过短、key 是否过于离散、是否有大量一次性访问。配合 \`慢日志\` 找出穿透到 DB 的慢查询，针对性加缓存。
`,
    code: `// =============================================================
