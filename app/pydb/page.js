...
//   1. 数据源：pydbChapters / pydbChapterGroups（来自 pydb-tutorial-data）
//   2. 运行接口：/api/run-py（由 CodeBlock 内部调用）
//   3. 底部 demo 直接复用 CodeBlock 组件，与教程内代码块完全一致
// =============================================================

import { useState, useEffect, useRef, useCallback } from "react";
import { pydbChapters, pydbChapterGroups } from "../courses-data/pydb-tutorial-data";
...
export default function PyDbTutorial() {
  ...
  const initialChapter = pydbChapters[0];
  ...
  useEffect(() => {
    ...
    if (!hash) return;
    const chapter = pydbChapters.find((c) => c.id === hash);
    if (chapter) {
