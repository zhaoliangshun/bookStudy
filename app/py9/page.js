...
//   1. 数据源：py9Chapters / py9ChapterGroups
...
export default function Py9Tutorial() {
  ...
  const getInitialChapterId = () => {
    if (typeof window !== "undefined") {
      ...
      if (hash && py9Chapters.find((c) => c.id === hash)) {
        return hash;
      }
    }
    return py9Chapters[0].id;
  };

  const initialId = getInitialChapterId();
  const initialChapter = py9Chapters.find((c) => c.id === initialId) || py9Chapters[0];
