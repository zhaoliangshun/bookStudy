...
// =============================================================
// Python asyncio 教程（pyasync）交互式页面
// -------------------------------------------------------------
// 专题教程：专注讲解 Python asyncio 异步编程的核心概念与日常开发应用
//   1. 数据源：pyasyncChapters / pyasyncChapterGroups（来自 pyasync-tutorial-data）
...
import { pyasyncChapters, pyasyncChapterGroups } from "../courses-data/pyasync-tutorial-data";
import { MarkdownRenderer } from "../MarkdownRenderer";
import Sidebar from "../components/Sidebar";
import CodeBlock from "../CodeBlock";

import { useReadingScrollPosition } from "../hooks/useReadingScrollPosition";
export default function PyAsyncTutorial() {
  // 默认使用第一个章节作为初始状态。
  ...
  const initialChapter = pyasyncChapters[0];
  ...
  // 当前章节对象
  const activeChapter =
    pyasyncChapters.find((c) => c.id === activeId) || pyasyncChapters[0];
  ...
  useEffect(() => {
    ...
    if (!hash) return;
    const chapter = pyasyncChapters.find((c) => c.id === hash);
    if (chapter) {
  ...
  const selectChapter = useCallback((chapterId) => {
    const chapter = pyasyncChapters.find((c) => c.id === chapterId);
    if (!chapter) return;
  ...
  // 按分组组织章节
  const groupedChapters = pyasyncChapterGroups.map((group) => ({
    group,
    items: pyasyncChapters.filter((c) => c.group === group),
  }));
  ...
  return (
    <div className="app-shell">
      <div className="main-layout">
        ...
        <Sidebar
          title="学习目录"
          tip="点击章节开始学习 Python asyncio 异步编程"
          ...
          currentPath="/pyasync"
          meta={`共 ${pyasyncChapters.length} 章 · 协程 + 实战 demo`}
        />
        ...
        <main className="content" ref={contentRef}>
          ...
          <footer className="content-footer">
            <p>
              Python asyncio 教程 · 代码由系统 python3 子进程执行 · 协程并发讲原理，demo 驱动学实战
            </p>
          </footer>
        </main>
      </div>
...
function ChapterNav({ activeId, onSelect }) {
  const idx = pyasyncChapters.findIndex((c) => c.id === activeId);
  const prev = idx > 0 ? pyasyncChapters[idx - 1] : null;
  const next = idx < pyasyncChapters.length - 1 ? pyasyncChapters[idx + 1] : null;
