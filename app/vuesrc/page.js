...
// Vue 源码构建教程页面
// -------------------------------------------------------------
// 主题：从零开始一步步构建 Vue 核心源代码
// 含代码编辑器与运行按钮，demo 在服务端 Node.js 沙箱中执行。
// 结构：侧边栏章节导航 + 主内容区 Markdown 渲染 + 代码块。
// =============================================================

import { useState, useEffect, useRef, useCallback } from "react";
import { vuesrcChapters, vuesrcChapterGroups } from "../courses-data/vuesrc-tutorial-data";
...
export default function VuesrcTutorial() {
  ...
  const initialChapter = vuesrcChapters[0];
  ...
  useEffect(() => {
    ...
    if (!hash) return;
    const chapter = vuesrcChapters.find((c) => c.id === hash);
    if (chapter) {
  ...
  const groupedChapters = vuesrcChapterGroups.map((group) => ({
    group,
    items: vuesrcChapters.filter((c) => c.group === group),
  }));
    ...
    <div className="app-shell">
      <div className="main-layout">
        ...
        <Sidebar
          ...
          currentPath="/vuesrc"
          meta={`共 ${vuesrcChapters.length} 章 · 源码构建`}
        />
        ...
        <main className="content" ref={contentRef}>
          ...
          <footer className="content-footer">
            <p>
              Vue 源码构建 · 从零构建核心源代码 · 代码在服务端沙箱中执行
            </p>
          </footer>
        </main>
      </div>
...
function ChapterNav({ activeId, onSelect }) {
  const idx = vuesrcChapters.findIndex((c) => c.id === activeId);
  const prev = idx > 0 ? vuesrcChapters[idx - 1] : null;
  const next = idx < vuesrcChapters.length - 1 ? vuesrcChapters[idx + 1] : null;
